MODULE MOD_USRFS
    ...
    !------------------------------------------------------------------
    IMPLICIT NONE
    REAL(KIND=8), PARAMETER::  dLOW  = 1.0D-30
    REAL(KIND=8), PARAMETER:: dHIGH = 1.0D+30
    ...
    PUBLIC :: fMSE  !! mean sum of squared errors
...
CONTAINS
    ...
    REAL(KIND=8) FUNCTION fWAVG(n, ww, xx, dFill)
        !!compute weighting average
        !!ARGUMENTS:
        INTEGER n
        REAL(KIND=8), DIMENSION(n) :: ww, xx
        REAL(KIND=8) dFill
        ...
        INTEGER j, k
        INTEGER jj(n)
        REAL(KIND=8), DIMENSION(:), ALLOCATABLE :: ww2
        k = 0
        do j = 1, n
    ...
    SUBROUTINE WOBJ_INI(nobj, wobj, nobj0, wobj0)
        ...
        !!ARGUMENTS:
        INTEGER nobj, nobj0
        REAL(KIND=8), DIMENSION(nobj) :: wobj
        REAL(KIND=8), DIMENSION(nobj0) :: wobj0
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fNSE(nArray, Dataobs, Datasim, dFill)
        ...
        !    REAL(KIND=8), parameter:: const_FillValue = -999d0
        !!ARGUMENTS:
        INTEGER, intent(in) :: nArray
        REAL(KIND=8) dFill
        REAL(KIND=8), DIMENSION(nArray) :: Dataobs, Datasim
        
        !!LOCAL VARIABLES:
        REAL(KIND=8) AVGobs, sum11, sum12
        INTEGER i, j, m
    ...
    END !!FUNCTION  
    !------------------------------------------------------------------
    REAL(KIND=8) function f1NSE(nArray, Dataobs, Datasim, dFill)
        !f1NSE = 1.0 - fNSE
    ...
    END !!FUNCTION 
    !------------------------------------------------------------------
    REAL(KIND=8) function f1NSE_norm(nArray, Dataobs, Datasim, dFill)
        !f1NSE = 1.0 - fNSE
        ...
        !!LOCAL VARIABLES:
        REAL(KIND=8) AVGobs, sum11, sum12
        REAL(KIND=8), ALLOCATABLE :: obs(:), sim(:)
        REAL(KIND=8) MINobs, MAXobs, MINsim, MAXsim
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fMARE_norm(nArray, Dataobs, Datasim, dFill)
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fMARE(nArray, Dataobs, Datasim, dFill)
        !!Mean Absolute Relative Error
        !	implicit REAL*8 (a-h,o-z)
        !	dimension Dataobs(1000),Datasim(1000)
        !!ARGUMENTS:
    ...
    REAL(KIND=8) function fMARE_tolerance(nArray, Dataobs, Datasim, tolerance, dFill)
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fSIGN(nArray, Dataobs, Datasim, dSign, dFill)
    ...
    !! dSign = -1 means ySim should be < yBase
        ...
        INTEGER nArray
        REAL(KIND=8), DIMENSION(nArray) :: Dataobs, Datasim
        REAL(KIND=8) :: dSign, dFill

        !!LOCAL VARIABLES:
        REAL(KIND=8) AVGobs, AVGsim, dtemp
    ...
    END
    !------------------------------------------------------------------
    REAL(KIND=8) function fMAP(nArray, Dataobs, Datasim, dFillValue)
        !Mean Absolute Percent Error (MAP), best MAP = 0, [Kothamasu et al., 2004]
        !data excluded from calculation if Dataobs(i) = const_FillValue 
        !    REAL(KIND=8), parameter:: const_FillValue = -999d0
        !!ARGUMENTS:
        INTEGER, intent(in) :: nArray
        REAL(KIND=8), intent(in) :: Dataobs(nArray), Datasim(nArray), dFillValue
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fSSE(nArray, Dataobs, Datasim, ifNorm, dFill)
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fMSE(nArray, Dataobs, Datasim, ifNorm, dFill)
        ...
        !!LOCAL VARIABLES:
        REAL(KIND=8) SSE
        SSE = fSSE(nArray, Dataobs, Datasim, ifNorm, dFill)
        fMSE = SSE/(nArray*1.d0)
        return
    END !! fMSE
    !------------------------------------------------------------------
    REAL(KIND=8) function fSSE_Normalize(nArray, Dataobs, Datasim, ifNorm, dFill)
        !!SSE/[2*VAR)
    ...
    END !!FUNCTION 
         ...
         !------------------------------------------------------------------
    REAL(KIND=8) function fMSE_Normalize(nArray, Dataobs, Datasim, ifNorm, dFill)
        !!1/m*SSE/[2*VAR)
        ...
        fMSE_Normalize = sum1/(2.d0*var1)/dble(m)
        return
    END !!FUNCTION 
    !------------------------------------------------------------------
    REAL(KIND=8) FUNCTION fNRMSE(nn, dobs, dobs_sd, dsim, dFill)
        !!Normalized Root-Mean-Square Error
        !!ARGUMENTS:      
        INTEGER nn
        REAL(KIND=8) dFill
        REAL(KIND=8), DIMENSION(nn) :: dobs, dobs_sd, dsim
        
        !!LOCAL VARIABLES:
        INTEGER j,k
        REAL(KIND=8) sum1, dobs_range
        REAL(KIND=8), PARAMETER :: dtol = 1.0d-6
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fSUM(nArray, xx, dFill)
        !!compute SUM of all elements in an Array. 
        !        implicit REAL(KIND=8)*8 (a-h,o-z)
        !!ARGUMENTS:
        INTEGER nArray
        REAL(KIND=8), DIMENSION(nArray) :: xx
        REAL(KIND=8) dFill
    ...
    END !!FUNCTION
    !------------------------------------------------------------------
    REAL(KIND=8) function fAVG(nArray, xx, dFill)                  
        !!compute SUM of all elements in an Array. 
        !        implicit REAL*8 (a-h,o-z)
        !!ARGUMENTS:
        INTEGER nArray
        REAL(KIND=8), DIMENSION(nArray) :: xx
        REAL(KIND=8) dFill,dx
    ...
    END
    !------------------------------------------------------------------
    REAL(KIND=8) function f1RAVG(nn, dobs, dsim, dFill)
        !!f1RAVG = abs(1.0 - fAVG_sim/fAVG_obs) 
        !        implicit REAL*8 (a-h,o-z)
        !!ARGUMENTS:
        INTEGER nn
        REAL(KIND=8), DIMENSION(nn) :: dobs, dsim
        REAL(KIND=8) dFill
        
        !!LOCAL VARIABLES:
        REAL(KIND=8) fAVG_obs, fAVG_sim
    ...
    END
    !------------------------------------------------------------------
    REAL(KIND=8) function f1RAVG_ratio(nn, dobs, dsim, ratio,dFill)
        !!f1RAVG = abs(1.0 - fAVG_sim/fAVG_obs) 
        !        implicit REAL*8 (a-h,o-z)
        !!ARGUMENTS:
        INTEGER nn
        REAL(KIND=8), DIMENSION(nn) :: dobs, dsim
        REAL(KIND=8) dFill, ratio
    ...
    END
    !------------------------------------------------------------------	
    REAL(KIND=8) function fSUM2(nArray, xx, iBegin, iEnd, dFill)
    ...
    END
    !------------------------------------------------------------------	
    REAL(KIND=8) function fAVG2(nArray, xx, iBegin, iEnd, dFill)
        !!compute AVERAGE of selected elements (iBegin to iEnd) in an Array. 
        !        implicit REAL*8 (a-h,o-z)
        !!ARGUMENTS:
        INTEGER nArray, iBegin, iEnd
    ...
    REAL(KIND=8) function fLxy(nArray, xx, yy, dFill)
        !!ARGUMENTS:
        INTEGER nArray
        REAL(KIND=8), DIMENSION(nArray) :: xx, yy
        REAL(KIND=8) dFill
        
        !!LOCAL VARIABLES:
        REAL(KIND=8), DIMENSION(nArray) :: xx1, yy1
        INTEGER jua, k   
        REAL(KIND=8) sumxy, sumx, sumy
        !        REAL(KIND=8) fSUM2 !!FUNCTION
    ...
    END
    !------------------------------------------------------------------
    REAL(KIND=8) function fCORR(nArray, xx, yy, dFill)
    ...
    END
    !------------------------------------------------------------------
    REAL(KIND=8) function f1CORR(nArray, xx0, yy0, iLog,dFill)
        !!1-CORRELATION COEFFICIENT
        !!ARGUMENTS:
        !! iLog = 1: log10 transformation
        INTEGER nArray, iLog
        REAL(KIND=8), DIMENSION(nArray) :: xx0, yy0
        REAL(KIND=8) dFill
    ...
    END
    !------------------------------------------------------------------      
    REAL(KIND=8) function fVARIANCE(n, xx, dFill)
        !!ARGUMENTS:
        INTEGER n
    ...
    !------------------------------------------------------------------      
    REAL(KIND=8) function fSTDDEV(n, xx, dFill)                 
        !!standard deviation
        !!ARGUMENTS:
        INTEGER n
        REAL(KIND=8), DIMENSION(n) :: xx
    ...
    !------------------------------------------------------------------      
    REAL(KIND=8) function fSTDERR(n, xx, dFill)
        !!standard error
        !!ARGUMENTS:
        INTEGER n
        REAL(KIND=8), DIMENSION(n) :: xx
    ...
    REAL(KIND=8) FUNCTION fMAXarray(nn, dobs, dFill)
        !!ARGUMENTS:
        REAL(KIND=8) dFill
        INTEGER nn
        REAL(KIND=8), DIMENSION(nn) :: dobs
    ...
    END !!FUNCTION
    !------------------------------------------------------------------
    REAL(KIND=8) FUNCTION fMINarray(nn, dobs, dFill)
        !!ARGUMENTS:
        REAL(KIND=8) dFill
        INTEGER nn
        REAL(KIND=8), DIMENSION(nn) :: dobs
    ...
    END !!FUNCTION
    !------------------------------------------------------------------
    REAL(KIND=8) FUNCTION fRANGEarray(nn, dobs, dFill)
        !!Normalized Root-Mean-Square Error
        !!ARGUMENTS:
        REAL(KIND=8) dFill
        INTEGER nn
        ...
        !!LOCAL VARIABLES
        REAL(KIND=8) dobs_max, dobs_min !!dobs_range
    ...
    !------------------------------------------------------------------
    SUBROUTINE Array_2dto1d(nrow, ncol, array2d, array1d)
        !!convert a 2d-Array to 1d-Array
        !!ARGUMENTS:
        INTEGER, intent(in) :: nrow, ncol
        REAL(KIND=8), intent(in) :: array2d(nrow, ncol)
        REAL(KIND=8), intent(out):: array1d(nrow*ncol)
        ...
        do i = 1, nrow
            ibeg = (i-1)*ncol + 1
            iend = i*ncol
            array1d(ibeg:iend) = array2d(i,1:ncol)
        end do
        return
    END !!

    !------------------------------------------------------------------
    SUBROUTINE Array_1dto2d(array1d, nrow, ncol, array2d)
        !!convert a 1d-Array to 2d-Array
        !!ARGUMENTS:
        INTEGER, intent(in) :: nrow, ncol
        REAL(KIND=8), intent(out) :: array2d(nrow, ncol)
        REAL(KIND=8), intent(in):: array1d(nrow*ncol)
        ...
        do i = 1, nrow
            ibeg = (i-1)*ncol + 1
            iend = i*ncol
            array2d(i,1:ncol) = array1d(ibeg:iend)
        end do
        return
    END !!
    ...
    SUBROUTINE selectINT(npg, m, lcs)
        ...
        if (m .ge. npg) then
            ...
            lcs(1) = 1 + int(npg + 0.5 - sqrt((npg + .5)**2 - &
                npg * (npg + 1) * randx))
            do k = 2, m
60          call random_number(randx) !randx = rand() !rand = ran1(iseed1)
    ...
    SUBROUTINE sort(n,m,rb,ra)
        ...
        !!ARGUMENTS:
        INTEGER n,m
        !          REAL(KIND=8) ra(n),rb(2000,m)
        REAL(KIND=8) ra(n),rb(:,:)

        !!LOCAL VARIABLES
        !          REAL(KIND=8) wk(2000,m)
...
200   Return
  ...
  subroutine sInt2Str(inum_in,nLen,str_out)
	...
	CHARACTER(len=100) sFormat,str_tmpr,str_tmpl
    INTEGER,      intent(in) :: inum_in,nLen
    INTEGER len_left
	write(sFormat,*)"(I",nLen,")"
	write(str_tmpr,sFormat) inum_in
	len_left = nLen - len_trim(adjustl(str_tmpr))
	if(len_left .eq. 0) then
		str_out = str_tmpr
	else 
		write(sFormat,*) "(",len_left,"A1)"
		write(str_tmpl,sFormat) "0"
		str_out = trim(adjustl(str_tmpl))//trim(adjustl(str_tmpr))
	end if
	!write(*,*) str_out
  end

  Subroutine sInt2Str0(inum_in,nLen,str_out)
      !!ARGUMENTS:
      CHARACTER(*), intent(out):: str_out
      INTEGER,      intent(in) :: inum_in,nLen
      ...
      !        if(inum_in < 1) write(*,*)inum_in, nLen, ndigit, nzero, str_out,sFormat
      return
  END !!subroutine sDate2YMD
  !------------------------------------------------------------------
  Subroutine sDateTime2Str(time,str_time)
      INTEGER time(8)
      CHARACTER(LEN=15)str_time !!yyyymmdd-hhmmss
      CHARACTER(Len=2)mon,day,hour,minute,second


      CALL DATE_AND_TIME(values = time)
      CALL sInt2Str(time(2),2,mon)
      CALL sInt2Str(time(3),2,day)
      CALL sInt2Str(time(5),2,hour)
      CALL sInt2Str(time(6),2,minute)
      CALL sInt2Str(time(7),2,second)
      write(str_time,'(I4,2A2,A1,3A2)')time(1),mon,day,"-",hour,minute,second
      return

  END
  !------------------------------------------------------------------
  SUBROUTINE sFile_Backup(sFile)
      CHARACTER(*) sFile
      CHARACTER(LEN=500) cmd

      LOGICAL file_exist

      INQUIRE(FILE=trim(sFile), EXIST=file_exist)

      if(file_exist) then
          cmd = "mv "//trim(sFile)//" "//trim(sFile)//"0"
  ...
  REAL(KIND=8) FUNCTION gasdev()
      ...
      !!LOCAL VARIABLES:
      REAL(KIND=8) v1,v2,r,fac,gset,randx
      INTEGER iset
      DATA iset / 0 /
...
1 v1 = (2. * randx) - 1.
  ...
  REAL(KIND=8) FUNCTION ran1(idum)
      ...
      REAL(KIND=8), PARAMETER :: rm1 = 3.8580247e-6, rm2 = 7.4373773e-6
...
11        continue
  ...
  !!------------------------------------------------------------------
  REAL(KIND=8) FUNCTION gasdev0(idum)
      !$debug
      !c
      !c
      !c  THIS SUBROUTINE IS FROM "NUMERICAL RECIPES" BY PRESS ET AL.
