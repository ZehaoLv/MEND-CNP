MODULE MOD_MEND  
    ...
    PRIVATE:: subMEND_PPOOL_UPDATE1
    PRIVATE:: subMEND_CN_UPDATE0
    PRIVATE:: subMEND_CN_UPDATE
    PRIVATE:: subMEND_CP_UPDATE0
    PRIVATE:: subMEND_CP_UPDATE
    PRIVATE:: sOUT_OPT_VAR_Extract
    PRIVATE:: fCN_UPDATE
    PRIVATE:: fCP_UPDATE
    ...
    PUBLIC :: sCOFI_PAR
    PUBLIC :: sSOBOL_VAR_Read
    PUBLIC :: sSOBOL_VAR_OBJ0
    PUBLIC :: sSOBOL_VAR_OBJ
    PUBLIC :: subMEND_Files_Open
    PUBLIC :: subMEND_Files_Close
    

    PUBLIC:: fMEND_OBJ
    PUBLIC:: fV_MM     !!Michaelis-Menten Kinetics
    PUBLIC:: sAdsorption      !!adsorption-desorption
...
CONTAINS
    ...
    !=============================================================================
    !MEND: Microbial-Enzyme Decomposition (Decomposition, Nitrification, & Denitrification))
    !-----------------------------------------------------------------------------
    SUBROUTINE subMEND_INI(sINI)
        ...
        INTEGER i, j, k, lp
        INTEGER ifobs
        REAL(KIND=8) frISO(const_nISO) !!, frISOadd(const_nISO)
        REAL(KIND=8) frac(3), CN_DOM, rCN_LIG2LAB, CP_DOM ,rCP_LIG2LAB !!rCN_LIG2LAB = CN_LIGNIN/CN_LABILE  !####

        CHARACTER(len = 100) sRead!!propName(nProp) !name of 10 properties
        !    INTEGER iRead  !!iTemp
        REAL(KIND=8) rRead  !!rTemp
    
        if(sINI%iModel.eq.3) then !!Sobol analysis
            CALL subMEND_Files_Open(sINI)
        end if
        sINI%iError = 0  !! sINI%iError <0  indicates mass balance error
        ifobs = 2  !! file unit number
        !
        ...
        sINI % CP_MB_avg   = sINI%dINI(27) !#### 
        sINI % CP_MB_min    = sINI%dINI(28)!####
        sINI % CP_MB_max    = sINI%dINI(29)!####
        sINI % CP_ENZP      = sINI%dINI(31)!####
        sINI % CP_ENZM    = sINI%dINI(32)  !####
        !!fQOM = sINI%dINI(10)
        ...
        IF(.not.sINI%Carbon_only) THEN
            ...
            frac = (/sINP % CPOOL % POM(1)/(sINI%dINI(13)+sINI%dINI(12)),&                             !####
                sINP % CPOOL % POM(2)/(sINI%dINI(13)+sINI%dINI(12)), &                                 !####
                sINI%dINI(13)/(sINI%dINI(13)+sINI%dINI(12))/)                                          !####
            rRead = frac(1)/(1.D0/sINI%dINI(33) - frac(3)/sINI%dINI(35) - frac(2)/const_CP_Cellulose)  !####
             !#  
            sINP % PPOOL % POM(1)= sINP % CPOOL % POM(1)/rRead !!sINI%dINI(20)  !!lignin               !####
            sINP % PPOOL % POM(2)= sINP % CPOOL % POM(2)/const_CP_Cellulose  !!cellulose               !####
            sINP % PPOOL % MOM   = sINP % CPOOL % MOM/sINI%dINI(35)                                    !####
            sINP % PPOOL % QOM   = sINP % CPOOL % QOM/sINI%dINI(35)                                    !####
            sINP % PPOOL % DOM   = sINP % CPOOL % DOM/sINI%dINI(36)                                    !####
            sINP % PPOOL % DOMS   = sINP % CPOOL % DOMS/sINI%dINI(36)                                  !####
            sINP % PPOOL % DOMT   = sINP % CPOOL % DOMT/sINI%dINI(36)                                  !####
            sINP % PPOOL % MBA   = sINP % CPOOL % MBA/sINI%dINI(37)                                    !####
            sINP % PPOOL % MBD   = sINP % CPOOL % MBD/sINI%dINI(37)                                    !####
            sINP % PPOOL % ENZP  = sINP % CPOOL % ENZP/sINI % CP_ENZP                                  !####
            sINP % PPOOL % ENZM  = sINP % CPOOL % ENZM/sINI % CP_ENZM                                  !####
             !# 
            ...
            sINP % MNPOOL % N2 = 1d-2 !!sINP % MNPOOL % NO
             !# 
            sINP % MNPOOL % PIP = sINI%dINI(39)         !####
            sINP % MNPOOL % SP = sINI%dINI(40)         !####
            sINP % MNPOOL % LIP = sINI%dINI(41)        !####
            sINP % MNPOOL % DIP = sINI%dINI(42)        !####
            sINP % MNPOOL % SIP = sINI%dINI(43)        !####
            sINP % MNPOOL % OIP = sINI%dINI(44)        !####
            ...
            sINP % PPOOL % ENZNm = sINP % CPOOL % ENZNm/sINI % CP_ENZM          !####
            
            rRead = sINP % MNPOOL % LIP + sINP % MNPOOL % DIP                                !####
            rRead = rRead/(sum(sINP % PPOOL % POM) + sINP % PPOOL % MOM)                     !####
            sINP % CPOOL % PTASE = rRead*(sum(sINP % CPOOL % ENZP) + sINP % CPOOL % ENZM)    !####
            sINP % NPOOL % PTASE = sINP % CPOOL % PTASE/sINI % CN_ENZM                       !####
            sINP % PPOOL % PTASE = sINP % CPOOL % PTASE/sINI % CP_ENZM                       !####
        
            CALL subMEND_CPOOL_UPDATE1(sINP%CPOOL)  !!2019-04-28 
            CALL subMEND_NPOOL_UPDATE1(sINP%NPOOL, sINP%MNPOOL)  
            CALL subMEND_PPOOL_UPDATE1(sINP%PPOOL, sINP%MNPOOL)  
            CALL subMEND_CN_UPDATE0(sINP % CPOOL, sINP % NPOOL, sINP%MNPOOL, sINP % CN)  
            CALL subMEND_CP_UPDATE0(sINP % CPOOL, sINP % PPOOL, sINP%MNPOOL, sINP % CP)  
            sINP%CN0 = sINP % CN  !! Initial C:N Values
            sINP%CP0 = sINP % CP
           ...
           !# P                                                                                                             !####  
            rCP_LIG2LAB = sINI%dINI(26)                                                                                     !####
                                                                                                                            !####
            sINI % CP_LITT(2) = const_CP_Cellulose                                                                          !####
            sINI % CP_WOOD(2) = const_CP_Cellulose                                                                          !####
            sINI % CP_ROOT(2) = const_CP_Cellulose                                                                          !####
                                                                                                                            !####
            frac = sINI%SIN_frac                                                                                            !####
            sINI % CP_LITT(3) = (frac(1)/rCP_LIG2LAB + frac(3))/(1.D0/sINI%CP_LITT_avg - frac(2)/sINI % CP_LITT(2))         !####
            sINI % CP_LITT(1) = sINI % CP_LITT(3)*rCP_LIG2LAB                                                               !####
                                                                                                                            !####
            rRead = sum(sINI%SIN_other(1,:))                                                                                !####
            if(rRead.lt.1.D-20) then                                                                                        !####
                frac = 1.D0/3                                                                                               !####
            ...
            sINI % CP_WOOD(3) = (frac(1)/rCP_LIG2LAB + frac(3))/(1.D0/sINI%CP_WOOD_avg - frac(2)/sINI % CP_WOOD(2))         !####
            sINI % CP_WOOD(1) = sINI % CP_WOOD(3)*rCP_LIG2LAB                                                               !####
                                                                                                                            !####
            rRead = sum(sINI%SIN_other(2,:))                                                                                !####
            if(rRead.lt.1.D-20) then                                                                                        !####
                frac = 1.D0/3                                                                                               !####
                ...
                frac = sINI%SIN_other(2,:)/rRead                                                                            !####
                                                                                                                            ...
                                                                                                                            !####
            sINI % CP_ROOT(3) = (frac(1)/rCP_LIG2LAB + frac(3))/(1.D0/sINI%CP_ROOT_avg - frac(2)/sINI % CP_ROOT(2))         !####
            sINI % CP_ROOT(1) = sINI % CP_ROOT(3)*rCP_LIG2LAB                                                               !####
        ...
        if (sINI % iModel .eq. 0 .or. sINI % iModel .eq. 3) then !output results for model simulation
            ...
            write(sINI%iFout_VAR_hour, '(i10,200e20.3)') &
                0, sINP % CPOOL, sINP%MNPOOL, sINP%NPOOL,sINP%PPOOL, sINP%CN, sINP%CP !!,sINP % CPOOLI(1),sINP % CPOOLI(2)
        end if
    ...
    SUBROUTINE subMEND_CPOOL_UPDATE1(CPOOL)
        ...
        CPOOL % ENZ = CPOOL % ENZD + CPOOL % ENZNmt + CPOOL % PTASE      
    ...
    SUBROUTINE subMEND_NPOOL_UPDATE1(NPOOL, MNPOOL)
        ...
        NPOOL % ENZ = NPOOL % ENZD + NPOOL % ENZNmt + NPOOL % PTASE  
    ...
    !-----------------------------------------------------------------------------
     SUBROUTINE subMEND_PPOOL_UPDATE1(PPOOL, MNPOOL)                          !####
        !!ARGUMENTS:                                                          !####
        TYPE(sMEND_POOL)    , intent(inout) :: PPOOL                          !####
        TYPE(sMEND_POOL_MN) , intent(inout) :: MNPOOL                         !####
                                                                              !####
        PPOOL%MB  = PPOOL%MBA + PPOOL%MBD                                     !####
        PPOOL%ENZD = sum(PPOOL%ENZP) + PPOOL%ENZM                             !####
                                                                              !####
        PPOOL%POMT = sum(PPOOL%POM)                                           !####
        PPOOL%MOMT = PPOOL%MOM + PPOOL%QOM                                    !####
        PPOOL%SOM = PPOOL%POMT + PPOOL%MOMT + PPOOL%DOMT                      !####
                                                                              !####
        PPOOL%ENZNmt = sum(PPOOL%ENZNm)                                       !####
        PPOOL%ENZ = PPOOL%ENZD + PPOOL%ENZNmt + PPOOL%PTASE                   !####
                                                                              !####
        !    NPOOL%DOMT = NPOOL%DOM + NPOOL%DOMS                              !####
        PPOOL%TOM = PPOOL%SOM + PPOOL%MB + PPOOL%ENZ                          !####
                                                                              !####
        PPOOL%TM  = PPOOL%TOM + &                                             !####
        MNPOOL%DIP+ MNPOOL%LIP + MNPOOL%OIP + MNPOOL%SIP + MNPOOL%PIP !+ MNPOOL%P_dep 
       ! write(*,*)PPOOL%MB,PPOOL%MBA,PPOOL%MBD
       ! write(*,*)PPOOL%POMT,PPOOL%MOMT,PPOOL%DOMT,PPOOL%SOM
       ! write(*,*)PPOOL%ENZD,PPOOL%ENZNmt,PPOOL%PTASE,PPOOL%ENZ
       ! write(*,*)PPOOL%TOM,PPOOL%TM
       ! write(*,*)MNPOOL%DIP,MNPOOL%LIP,MNPOOL%OIP,MNPOOL%SIP,MNPOOL%PIP
    END !!SUBROUTINE subMEND_POOL_UPDATE1(PPOOL)
    !-----------------------------------------------------------------------------
    REAL(KIND=8) FUNCTION fCN_UPDATE(CPOOL, NPOOL, CN0)
        !! Calculate C:N ratios in SOM pools
        REAL(KIND=8) CPOOL, NPOOL, CN0

        !! avoid CN = 0, as CN will be used later to calculate N flux = C flux/CN
        if(CPOOL.le.0.d0 .or. NPOOL.le.0.d0) then
            fCN_UPDATE = CN0
        else
            fCN_UPDATE = CPOOL/NPOOL
        end if
    ...
    !-----------------------------------------------------------------------------
    REAL(KIND=8) FUNCTION fCP_UPDATE(CPOOL, PPOOL, CP0)
        !! Calculate C:N ratios in SOM pools
        REAL(KIND=8) CPOOL, PPOOL, CP0

        !! avoid CN = 0, as CN will be used later to calculate N flux = C flux/CN
        if(CPOOL.le.0.d0 .or. PPOOL.le.0.d0) then
            fCP_UPDATE = CP0
        else
            fCP_UPDATE = CPOOL/PPOOL
        end if
    ...
    SUBROUTINE subMEND_CN_UPDATE(CPOOL, NPOOL, MNPOOL, CN0, CN)
        ...
        CN % TM     = fCN_UPDATE(CPOOL % TM, (NPOOL % TM - MNPOOL%NGas), CN0%TM)
        CN % TOM    = fCN_UPDATE(CPOOL % TOM, NPOOL % TOM, CN0%TOM)
        CN % SOM    = fCN_UPDATE(CPOOL % SOM, NPOOL % SOM, CN0%SOM)
        do i = 1, const_nPOM
            CN % POM(i)    = fCN_UPDATE(CPOOL % POM(i), NPOOL % POM(i), CN0%POM(i))
            CN % ENZP(i)   = fCN_UPDATE(CPOOL % ENZP(i), NPOOL % ENZP(i), CN0%ENZP(i))
        end do
        CN % POMT   = fCN_UPDATE(CPOOL % POMT, NPOOL % POMT, CN0%POMT)
        CN % MOMT   = fCN_UPDATE(CPOOL % MOMT, NPOOL % MOMT, CN0%MOMT)
        CN % MOM    = fCN_UPDATE(CPOOL % MOM, NPOOL % MOM, CN0%MOM)
        CN % QOM    = fCN_UPDATE(CPOOL % QOM, NPOOL % QOM, CN0%QOM)
        CN % DOM    = fCN_UPDATE(CPOOL % DOM, NPOOL % DOM, CN0%DOM)
        CN % DOMS   = CN % DOM !!CPOOL % DOMS/NPOOL % DOMS
        CN % DOMT   = CN % DOM !!CPOOL % DOMT/NPOOL % DOMT
        CN % MB     = fCN_UPDATE(CPOOL % MB, NPOOL % MB, CN0%MB)
        CN % MBA    = fCN_UPDATE(CPOOL % MBA, NPOOL % MBA, CN0%MBA)
        CN % MBD    = fCN_UPDATE(CPOOL % MBD, NPOOL % MBD, CN0%MBD)
        CN % ENZ    = fCN_UPDATE(CPOOL % ENZ, NPOOL % ENZ, CN0%ENZ)
        CN % ENZD   = fCN_UPDATE(CPOOL % ENZD, NPOOL % ENZD, CN0%ENZD)
        CN % ENZM   = fCN_UPDATE(CPOOL % ENZM, NPOOL % ENZM, CN0%ENZM)
        CN % PTASE  = fCN_UPDATE(CPOOL % PTASE, NPOOL % PTASE, CN0%PTASE)
        do i = 1, const_nENZNm
            CN % ENZNm(i)  = fCN_UPDATE(CPOOL % ENZNm(i), NPOOL % ENZNm(i), CN0%ENZNm(i))
        end do

        CN % ENZNmt = fCN_UPDATE(CPOOL % ENZNmt, NPOOL % ENZNmt, CN0%ENZNmt)
        CN % TM_err = CN % TM

    END !!SUBROUTINE subMEND_CN_UPDATE
    !-----------------------------------------------------------------------------
    SUBROUTINE subMEND_CP_UPDATE(CPOOL, PPOOL, MNPOOL, CP0, CP)
        !!ARGUMENTS:
        TYPE(sMEND_POOL), intent(in)    :: CPOOL
        TYPE(sMEND_POOL), intent(in)    :: PPOOL
        TYPE(sMEND_POOL_MN), intent(in) :: MNPOOL
        TYPE(sMEND_POOL), intent(IN)    :: CP0
        TYPE(sMEND_POOL), intent(OUT)   :: CP
        ...
        CP % TM     = fCP_UPDATE(CPOOL % TM,   PPOOL % TM, CP0%TM)
        CP % TOM    = fCP_UPDATE(CPOOL % TOM, PPOOL % TOM, CP0%TOM)
        CP % SOM    = fCP_UPDATE(CPOOL % SOM, PPOOL % SOM, CP0%SOM)
        do i = 1, const_nPOM
            CP % POM(i)    = fCP_UPDATE(CPOOL % POM(i), PPOOL % POM(i), CP0%POM(i))
            CP % ENZP(i)   = fCP_UPDATE(CPOOL % ENZP(i), PPOOL % ENZP(i), CP0%ENZP(i))
        end do
        CP % POMT   = fCP_UPDATE(CPOOL % POMT, PPOOL % POMT, CP0%POMT)
        CP % MOMT   = fCP_UPDATE(CPOOL % MOMT, PPOOL % MOMT, CP0%MOMT)
        CP % MOM    = fCP_UPDATE(CPOOL % MOM, PPOOL % MOM, CP0%MOM)
        CP % QOM    = fCP_UPDATE(CPOOL % QOM, PPOOL % QOM, CP0%QOM)
        CP % DOM    = fCP_UPDATE(CPOOL % DOM, PPOOL % DOM, CP0%DOM)
        CP % DOMS   = CP % DOM !!CPOOL % DOMS/NPOOL % DOMS
        CP % DOMT   = CP % DOM !!CPOOL % DOMT/NPOOL % DOMT
        CP % MB     = fCP_UPDATE(CPOOL % MB, PPOOL % MB, CP0%MB)
        CP % MBA    = fCP_UPDATE(CPOOL % MBA, PPOOL % MBA, CP0%MBA)
        CP % MBD    = fCP_UPDATE(CPOOL % MBD, PPOOL % MBD, CP0%MBD)
        CP % ENZ    = fCP_UPDATE(CPOOL % ENZ, PPOOL % ENZ, CP0%ENZ)
        CP % ENZD   = fCP_UPDATE(CPOOL % ENZD, PPOOL % ENZD, CP0%ENZD)
        CP % ENZM   = fCP_UPDATE(CPOOL % ENZM, PPOOL % ENZM, CP0%ENZM)

        do i = 1, const_nENZNm
            CP % ENZNm(i)  = fCP_UPDATE(CPOOL % ENZNm(i), PPOOL % ENZNm(i), CP0%ENZNm(i))
        end do

        CP % ENZNmt = fCP_UPDATE(CPOOL % ENZNmt, PPOOL % ENZNmt, CP0%ENZNmt)
        CP % TM_err = CP % TM
        CP % PTASE  = fCP_UPDATE(CPOOL % PTASE, PPOOL % PTASE ,CP0%PTASE)
    END !!SUBROUTINE subMEND_CN_UPDATE
    !-----------------------------------------------------------------------------
    SUBROUTINE subMEND_CN_UPDATE0(CPOOL, NPOOL, MNPOOL,CN)
        !!ARGUMENTS:
        TYPE(sMEND_POOL), intent(in)    :: CPOOL
        TYPE(sMEND_POOL), intent(in)    :: NPOOL
        TYPE(sMEND_POOL_MN), intent(in) :: MNPOOL
        ...
        CN % PTASE  = CPOOL % PTASE/NPOOL % PTASE                         
        CN % TM_err = CN % TM
        

    END !!SUBROUTINE subMEND_CN_UPDATE
    !-----------------------------------------------------------------------------
    SUBROUTINE subMEND_CP_UPDATE0(CPOOL, PPOOL, MNPOOL,CP)                 !#### 
        !!ARGUMENTS:                                                       !#### 
        TYPE(sMEND_POOL), intent(in)    :: CPOOL                           !#### 
        TYPE(sMEND_POOL), intent(in)    :: PPOOL                           !#### 
        TYPE(sMEND_POOL_MN), intent(in) :: MNPOOL                          !#### 
        ...
        CP % TM     = CPOOL % TM/(PPOOL % TM)                              !#### 
        CP % TOM    = CPOOL % TOM/PPOOL % TOM                              !#### 
        CP % SOM    = CPOOL % SOM/PPOOL % SOM                              !#### 
        CP % POM    = CPOOL % POM/PPOOL % POM                              !#### 
        CP % POMT    = CPOOL % POMT/PPOOL % POMT                           !#### 
        CP % MOMT    = CPOOL % MOMT/PPOOL % MOMT                           !#### 
        CP % MOM    = CPOOL % MOM/PPOOL % MOM                              !#### 
        CP % QOM    = CPOOL % QOM/PPOOL % QOM                              !#### 
        CP % DOM    = CPOOL % DOM/PPOOL % DOM                              !#### 
        CP % DOMS    = CP % DOM !!CPOOL % DOMS/PPOOL % DOMS                !#### 
        CP % DOMT    = CP % DOM !!CPOOL % DOMT/PPOOL % DOMT                !#### 
        CP % MB     = CPOOL % MB/PPOOL % MB                                !#### 
        CP % MBA    = CPOOL % MBA/PPOOL % MBA                              !#### 
        CP % MBD    = CPOOL % MBD/PPOOL % MBD                              !#### 
        CP % ENZ    = CPOOL % ENZ/PPOOL % ENZ                              !#### 
        CP % ENZD    = CPOOL % ENZD/PPOOL % ENZD                           !#### 
        CP % ENZP   = CPOOL % ENZP/PPOOL % ENZP                            !#### 
        CP % ENZM   = CPOOL % ENZM/PPOOL % ENZM                            !#### 
        CP % ENZNm  = CPOOL % ENZNm/PPOOL % ENZNm                          !#### 
        CP % ENZNmt  = CPOOL % ENZNmt/PPOOL % ENZNmt                       !#### 
        CP % PTASE  = CPOOL % PTASE/PPOOL % PTASE                          !#### 
        CP % TM_err = CP % TM                                              !#### 
END
    !-----------------------------------------------------------------------------
    
    REAL(KIND=8) function fMEND_OBJ(xx, sPAR, sINI, sOUT)       
        !    USE MOD_MEND
        !!ARGUMENTS:
        TYPE(sMEND_PAR), intent(inout) :: sPAR
        TYPE(sMEND_INI), intent(inOut) :: sINI
        TYPE(sMEND_OUT), intent(inOut) :: sOUT
        REAL(KIND=8)        , intent(in)    :: xx(sINI%nPar)
    
        !!LCOAL VARIABLES:
        !    TYPE(sMEND_INP) sINP
        !    REAL(KIND=8) sum1, sum2, stddev
        !    INTEGER nObj
        ...
        if (sINI%iError < 0) then                  
            sINI%rOBJ = dabs(const_FillValue)
            fMEND_OBJ = dabs(const_FillValue)       
            return
        end if
    ...
    SUBROUTINE subMEND_output(sDate,ihr, sPAR, sINI, sOUT)                
        ...
        write(sINI%iFout_VAR_hour, '(A10,200e20.6)') sDateHr, sOUT % CPOOL, sOUT%MNPOOL, sOUT%NPOOL, sOUT%PPOOL, sOUT%CN, sOUT%CP 
        write(sINI%iFout_FLX_hour, '(A10,200e20.6)') sDateHr, sOUT % CFLUX, sOUT%MNFLUX, sOUT%NFLUX, sOUT%PFLUX                   
    ...
    SUBROUTINE subMEND_output_rate(sDate,ihr, sINI, sPAR, sINP, sOUT)                   
        ...
        !!LOCAL VARIABLES            
        REAL(KIND=8) kSOM           !!Equivalent 1st-order decomposition rate; k=Vd*ENZP/(POM + Ks)
        REAL(KIND=8) kPOM1           !!Equivalent 1st-order decomposition rate; k=Vd*ENZP/(POM + Ks)
        REAL(KIND=8) kPOM2           !!Equivalent 1st-order decomposition rate; k=Vd*ENZP/(POM + Ks)
        REAL(KIND=8) kPOM           !!Equivalent 1st-order decomposition rate; k=Vd*ENZP/(POM + Ks)
        REAL(KIND=8) kMOM            !!Equivalent 1st-order decomposition rate; k=MOMdec/(MOM+QOM)
        REAL(KIND=8) kQOM            !!Equivalent 1st-order decomposition rate; k=Vd*ENZM/(QOM + Ks)
        REAL(KIND=8) kDOM            !!Equivalent 1st-order turnover rate of DOM; k=[(Vg+Vm)/Yg]*MB/(DOM + Ks)
        REAL(KIND=8) kMBa            !!Equivalent 1st-order turnover rate of Active Mcirobes; k=respiration_rate + mortality_rate+Enzyme_production_rate
        REAL(KIND=8) kMBa_in         !!Equivalent 1st-order assimilation rate of active microbes
        REAL(KIND=8) kMBd            !!Equivalent 1st-order turnover rate of dormant microbes
        REAL(KIND=8) kMBd_in         !!Equivalent 1st-order microbial dormancy rate
        REAL(KIND=8) kMB             !!Equivalent 1st-order turnover rate of microbes
        REAL(KIND=8) kMB_in          !!Equivalent 1st-order assimilation rate of total microbes
        REAL(KIND=8) phi             !!DOM saturation level; phi=DOM/(DOM+Ks)
        REAL(KIND=8) rMBa            !!Active Fraction of Microbes, r=MBA/MB
        REAL(KIND=8) CUE             !!Apparent Microbial Carbon Use Efficiency, CUE=[DOM_to_MBA - CO2_gmo]/DOM_to_MBA
        REAL(KIND=8) NUE             !!Apparent Nitrogen Use Efficiency, NUE=YgN
        REAL(KIND=8) rNH4            !! = NH4/NH4tot
    
        REAL(KIND=8) EA_POM(const_nPOM) !!Eyzyme Activity = Vmax * Enzyme Concentration
        REAL(KIND=8) EA_MOM
        REAL(KIND=8) EA_NFix
        REAL(KIND=8) EA_Nit
        REAL(KIND=8) EA_Denit(const_nDenit)
    
        REAL(KIND=8) kNFix           !!Equivalent 1st-order N Fixation rate = Flux/N2
        REAL(KIND=8) kNit            !!Equivalent 1st-order Nitrification rate = Flux/NH4
        REAL(KIND=8) kDenit(const_nDenit)    !!Equivalent 1st-order Denitrification rate = Flux/[NO3 or NO2 or NO or N2O]
    
    
        CHARACTER(LEN=2)  str2
        CHARACTER(LEN=10) sDateHr
    
        REAL(KIND=8), PARAMETER :: rMAX = 1.d-20

        CALL sInt2Str(ihr,2,str2)
        sDateHr = sDate//str2
    
        phi     = sINP%CPOOL%DOM/(sPAR%KsDOM+sINP%CPOOL%DOM)
        rMBa    = sINP%CPOOL%MBA/max(rMAX,sINP%CPOOL%MB)
        ...
        kMBa_in = (sPAR%Vg + sPAR%Vm)/sPAR%Yg*phi + sOUT%CFLUX%MBD_to_MBA/max(rMAX,sINP%CPOOL%MBA)
        kMBd_in = sOUT%CFLUX%MBA_to_MBD/max(rMAX,sINP%CPOOL%MBD)
        kMBd    = (sOUT%CFLUX%MBD_to_MBA + sOUT%MNFLUX%CO2_maintn_MBD)/max(rMAX,sINP%CPOOL%MBD)
        kMB     = (sOUT%MNFLUX%CO2_gm + sOUT%CFLUX%MBA_PM)/max(rMAX,sINP%CPOOL%MB)
        kMB_in  = sOUT%CFLUX%DOM_to_MBA/max(rMAX,sINP%CPOOL%MB)
        if (sOUT%CFLUX%DOM_to_MBA > rMAX) then
            CUE = (sOUT%CFLUX%DOM_to_MBA - sOUT%MNFLUX%CO2_gmo)/sOUT%CFLUX%DOM_to_MBA
        else
            CUE = 0.D0
        end if
        NUE     = sPAR%YgN
        rNH4    = sOUT%MNPOOL%NH4/max(rMAX,sOUT%MNPOOL%NH4tot)
        ...
        SELECT CASE (sINI%iKinetics)
            ...
            CASE DEFAULT !!M-M Kinetics, iType = 0, 10
                kSOM  = (sum(sOUT%CFLUX%POMdec_to_DOM) + sOUT%CFLUX%MOMdec)/ &
                    max(rMAX,(sINP%CPOOL%POMT + sINP%CPOOL%MOMT))
                kPOM  = sum(sOUT%CFLUX%POMdec)/max(rMAX,sINP%CPOOL%POMT)
                ...
                kMOM  = sOUT%CFLUX%MOMdec/max(rMAX,sINP%CPOOL%MOMT)
                ...
                kNFix = sOUT%MNFLUX%NFix/max(rMAX,sINP%MNPOOL%N2)
                kNit  = sOUT%MNFLUX%Nitrif/max(rMAX,sINP%MNPOOL%NH4tot)
                kDenit(1)  = sOUT%MNFLUX%Denit(1)/max(rMAX,sINP%MNPOOL%NO3)
                kDenit(2)  = sOUT%MNFLUX%Denit(2)/max(rMAX,sINP%MNPOOL%NO2)
                kDenit(3)  = sOUT%MNFLUX%Denit(3)/max(rMAX,sINP%MNPOOL%NO)
                kDenit(4)  = sOUT%MNFLUX%Denit(4)/max(rMAX,sINP%MNPOOL%N2O)
            
        END SELECT
        !    if (sINI%iModel .eq. 0) then
        write(sINI%iFout_rate_hour, '(A10,100e20.3)') &
            sDateHr, kSOM,kPOM1,kPOM2,kPOM,kMOM,kQOM,kDOM,kMBa,kMBa_in,kMBd,kMBd_in,kMB,kMB_in,&
            phi,rMBa, CUE, NUE,rNH4,sINI%fDOM,sINI%fNO3_Leaching,sINI%fPO4_Leaching, sINI%VNup_VG_GPPscaler,&
    ...
    SUBROUTINE subMEND_RUN(xx, sPAR, sINI, sOUT)         
        ...
        TYPE(sMEND_INP) sINP
        INTEGER*4 i, k
        INTEGER j, lp, tstep, ibeg, iend, iday, jbeg !! jend
        !    INTEGER iObs_time
        REAL(KIND=8) frISOadd(const_nISO)
        INTEGER nHour,nday,nmon
        REAL(KIND=8), ALLOCATABLE:: dSIM_d(:,:)  !!daily
        REAL(KIND=8), ALLOCATABLE:: dSIM_m(:,:)  !!monthly
        INTEGER nOBS
    
        CHARACTER(len=6) sYM
        CHARACTER(len=8) sDate
        INTEGER iYYYYMM, iyr,imo,ida,ihr, iyr_end_all
        REAL(KIND=8) dSIM_h(sINI%nHour_sim,sINI%nVARopt)  !!store hourly output
        REAL(KIND=8) dSIM(24,sINI%nVARopt)  !!store hourly output
        ...
        INTEGER iJDay
        REAL(KIND=8) SIN0,STP0,SWC0,SWP0,SpH0
        REAL(KIND=8) SIN_mean(const_nHourInYear)
        REAL(KIND=8) STP_mean(const_nHourInYear)
        REAL(KIND=8) SWC_mean(const_nHourInYear)
        REAL(KIND=8) SWP_mean(const_nHourInYear)
        REAL(KIND=8) SpH_mean(const_nHourInYear)
        REAL(KIND=8) SIN3_mean(const_nHourInYear)
        ...
        if(nday.gt.0 .and. sINI%iScenario == 1) then
            ...
            do j = 1, const_nHourInYear
                read(9001,*)sRead,SIN_mean(j),STP_mean(j),SWC_mean(j),SWP_mean(j),SpH_mean(j),sIN3_mean(j)
            end do
            close(9001)
        end if
        ...
        !![10/15/2021] change to read data in file
...
!        jend = 24*nDaysbwDates(sINI%sDate_beg_sim,sINI%sDate_end_inp2)
        ...
        do iday = 1,nday
            ...
            do lp = 1, 24  !!        
                ...
                sINP % PPOOL    = sOUT % PPOOL      
                sINP % MNPOOL   = sOUT % MNPOOL
                sINP%CN       = sOUT%CN
                sINP%CP       = sOUT%CP
                if(.not.(sINI%Carbon_only) .and. isnan(sOUT%CN%MBA)) then
            ...
            !        write(1010,'(70F30.20)') xx
                ...
                if(k.le.sINI%nHour) then
                    ...
                    !! wgs:beg:6/9/2020:
                    !#! sPAR%fINP is assigned in subMEND_PAR, and subMEND_PAR requires sINP%tmp, SWC, SWP, pH
                    !! It does NOT matter: at the first time-step, i.e., first hour, sPAR%fINP = 0
                    !! sPAR%fINP will NOT be assigned a value until CALL subMEND_PAR(xx, sPAR, sINI) at the end of the hour loop
                    !! we can solve the problem by implementing CALL subMEND_PAR() here,
                    !! However, we need to do the same thing for 'else !! POST-DATA PERIOD',
                    !! which makes the code too complicated
                    !! In conclusion, it's NOT worth doing this since the ERROR only affects the 1st time step
!!                    CALL subMEND_PAR(xx, sPAR, sINI)   !!modify parameter values by soil temperature, water potential, pH
                    ...
                    sINI%fNO3_Leaching = fracNO3_Leaching(sINP%SWC,sINI%porosity,sINI%SWCFC,sINI%Ksat,sINI%Lambda,&
                        sPAR%rNleach,sINI%soilDepth,1.d0)                                      
                    sINI%fPO4_Leaching = fracPO4_Leaching(sINP%SWC,sINI%porosity,sINI%SWCFC,sINI%Ksat,sINI%Lambda,&
                        sPAR%rPleach,sINI%soilDepth,1.d0)                                      
                    !!External INPUT: BEGIN
                    sOUT % CFLUX % POMadd(1) = sINP%SIN * sINI%SIN_frac(1) + sINI%SIN_other(1,1) + sINP%SIN3 * sINI%SIN3_frac(1)  !![mg POM/g soil/h], inputs to POM
                    sOUT % CFLUX % POMadd(2) = sINP%SIN * sINI%SIN_frac(2) + sINI%SIN_other(1,2) + sINP%SIN3 * sINI%SIN3_frac(2)
                    sOUT % CFLUX % DOMadd    = sINP%SIN * sINI%SIN_frac(3) + sINI%SIN_other(1,3) + sINP%SIN3 * sINI%SIN3_frac(3) !![mg DOM/g soil/h], inputs to DOM
...
!                    end if
                    ...
                    IF(.NOT.sINI%Carbon_only) THEN           
                        ...
                        sOUT % NFLUX % POMadd(1) = sOUT % NFLUX % POMadd(1) + sINP%SIN3 * sINI%SIN3_frac(1)/sINI%CN_ROOT(1)
                        sOUT % NFLUX % POMadd(2) = sOUT % NFLUX % POMadd(2) + sINP%SIN3 * sINI%SIN3_frac(2)/sINI%CN_ROOT(2)
                        sOUT % NFLUX % DOMadd    = sOUT % NFLUX % DOMadd    + sINP%SIN3 * sINI%SIN3_frac(3)/sINI%CN_ROOT(3)
                        !# P
                        sOUT % PFLUX % POMadd(1) = sINP%SIN * sINI%SIN_frac(1)/sINI%CP_LITT(1) + sINI%SIN_other(1,1)/sINI%CP_WOOD(1)  !!!####  
                        sOUT % PFLUX % POMadd(2) = sINP%SIN * sINI%SIN_frac(2)/sINI%CP_LITT(2) + sINI%SIN_other(1,2)/sINI%CP_WOOD(2)    !####
                        sOUT % PFLUX % DOMadd    = sINP%SIN * sINI%SIN_frac(3)/sINI%CP_LITT(3) + sINI%SIN_other(1,3)/sINI%CP_WOOD(3)    !####
                                                                                                                                        !####
                        sOUT % PFLUX % POMadd(1) = sOUT % PFLUX % POMadd(1) + sINP%SIN3 * sINI%SIN3_frac(1)/sINI%CP_ROOT(1)             !####
                        sOUT % PFLUX % POMadd(2) = sOUT % PFLUX % POMadd(2) + sINP%SIN3 * sINI%SIN3_frac(2)/sINI%CP_ROOT(2)             !####
                        sOUT % PFLUX % DOMadd    = sOUT % PFLUX % DOMadd    + sINP%SIN3 * sINI%SIN3_frac(3)/sINI%CP_ROOT(3)             !####
...
!                        end if
                        ...
                        sOUT%MNFLUX%P_dep   = sINI%SIN_P(k) 
                    ...
                    if(sINI%iScenario == 1) then
                        ...
                        j = (iJday - 1)*24 + lp
                        sINP%GPP = SIN_mean(j)
                        SIN0 = SIN_mean(j)*sPAR%fINP + SIN3_mean(j)
                        STP0 = STP_mean(j)
                        SWC0 = SWC_mean(j)
                        SWP0 = SWP_mean(j)
                        SpH0 = SpH_mean(j)
                    !!---------------------------------------------
                        ...
                        if(j==0) j=sINI%nHour
                        sINP%GPP = sINI%SIN(j)
                        SIN0 = sINI%SIN(j)*sPAR%fINP + sINI%SIN3(j)
                        STP0 = sINI%STP(j)
                        SWC0 = sINI%SWC(j)
                        SWP0 = sINI%SWP(j)
                        SpH0 = sINI%SpH(j)
                    ...
                    IF(.NOT.sINI%Carbon_only) THEN
                    !![10/15/2021] SIN0 = type1 + type3; as for long-term projection, currently don't consider difference in fractions between type1 and type3 inputs
                        ...
                        sOUT%MNFLUX%P_dep = sINI%SIN_P(j)   !!TODO
                    END IF
                end if
                ...
                !if (iday == 33 .and. lp == 15 )then
                !write(*,*) iday, lp
                !end if
                if  (sINI%iError < 0) then  
                write(*,*) sOUT%CPOOL%TM_err,iday, lp
                write(*,*) sOUT%NPOOL%TM_err,iday, lp
                write(*,*) sOUT%PPOOL%TM_err,iday, lp
                return   
                end if
                ...
                CALL sOUT_OPT_VAR_Extract(sINI%nVARopt,24,lp,dSIM,sPAR,sOUT,sINI%VARopt_int)  
        ...
        do j = 1,sINI%nVARopt                     
            i = sINI%VARopt_int(j,1)   !! Cali_varid, [10/13/2021: PROBLEM: this 'i' is NOT used later, and 'i' is assigned another value below]
            ...
            do k =1,nOBS                        
                ...
                if(tstep.eq.0) then  !!# hourly
                    ...
                    write(sDate,'(I8)')int(sINI%dOBS_opt(lp,1))  !!convert REAL(KIND=8) to CHARACTER(len=8)
                ...
                else if(tstep.eq.6) then  !!overall mean value of last 95% data
!                    rRead(1) = max(0.9,sINI%VARobj_tol(i))
                    rRead(1) = 0.05
                    ibeg = max(1,int(nday*rRead(1)))
                    iend = nday
                    sINI%dSIM_opt(lp,2) = fAVG((iend-ibeg+1),dSIM_d(ibeg:iend,j*2-1),const_FillValue)
                    sINI%dSIM_opt(lp,3) = fSTDDEV((iend-ibeg+1),dSIM_d(ibeg:iend,j*2-1),const_FillValue)
                end if
    ...
    SUBROUTINE subMEND(xx, sPAR, sINI, sOUT)        
        ...
        TYPE(sMEND_INI), intent(inout)  :: sINI
        TYPE(sMEND_OUT), intent(inout)  :: sOUT
        REAL(KIND=8)        , intent(in)     :: xx(sINI%nPar)  !!used in subMEND_PAR
        ...
        INTEGER i
        REAL(KIND=8) sumPOM, phi  !!OC1, OC2,, sumENZP
        REAL(KIND=8) frPOM(const_nPOM) !!, frENZP(const_nPOM) !!, ENZP_loss(const_nPOM) !!POMdec(const_nPOM),
        REAL(KIND=8) frMBA_to_POM(const_nPOM), frENZNm(const_nENZNm)
        REAL(KIND=8) rtp1, rtp2, rtp3, rtp4
        REAL(KIND=8) II(6),JJ,KK
        REAL(KIND=8)::ck_out        
        REAL(KIND=8)::bulk_den=1    
        
    
        !    ASSOCIATE(sINP => sINI%sINP)
!        frMBA_to_POM = (/0.1d0, 0.9d0/)  !! 10% entering POM_Oxidative, 90% entering POM_Hydrolytic
        frMBA_to_POM = (/sPAR%frMB2POMO, 1.0 - sPAR%frMB2POMO/)  !! 8/1/2020: 100% entering POM_Oxidative, 0% entering POM_Hydrolytic
        ...
        !        sINI%fDOM = 1.d0  !!DOM = DOMT
        !    end if
        !  sOUT%MNFLUX , sOUT%CFLUX, sOUT%NFLUX
        ...
        sOUT%PFLUX%TOTinp = sum(sOUT%PFLUX%POMadd) + sOUT%PFLUX%DOMadd + sOUT%MNFLUX%P_dep   
        ...
        IF(sINI%iKinetics.le.10) then
            ...
            CALL sAdsorption(sSorpPAR, sSorpINP, sSorpOUT)   
        ...
        !###===========================================================================   
           !POM(1)
           ck_out= sOUT%CFLUX%POMdec_to_MOM(1)+sOUT%CFLUX%POMdec_to_DOM(1)
           if(ck_out .gt. sOUT % CPOOL % POM(1)) then
		   sOUT%CFLUX%POMdec_to_MOM(1)=sOUT % CPOOL % POM(1)*sOUT%CFLUX%POMdec_to_MOM(1)/ck_out
		   sOUT%CFLUX%POMdec_to_DOM(1)=sOUT % CPOOL % POM(1)*sOUT%CFLUX%POMdec_to_DOM(1)/ck_out
           sOUT%CFLUX%POMdec(1)= sOUT%CFLUX%POMdec_to_MOM(1)+ sOUT%CFLUX%POMdec_to_DOM(1)
        !   write(*,*) 'callme'
		   end if
		   !POM(2)
		   ck_out= sOUT%CFLUX%POMdec_to_MOM(2)+sOUT%CFLUX%POMdec_to_DOM(2)
           if(ck_out .gt. sOUT % CPOOL % POM(2)) then
		   sOUT%CFLUX%POMdec_to_MOM(2)=sOUT % CPOOL % POM(2)*sOUT%CFLUX%POMdec_to_MOM(2)/ck_out
		   sOUT%CFLUX%POMdec_to_DOM(2)=sOUT % CPOOL % POM(2)*sOUT%CFLUX%POMdec_to_DOM(2)/ck_out
           sOUT%CFLUX%POMdec(2)= sOUT%CFLUX%POMdec_to_MOM(2)+ sOUT%CFLUX%POMdec_to_DOM(2)
         !  write(*,*) 'callme'
           end if
		   !=====================================================================================
		   !MOM
		   ck_out=sOUT%CFLUX%MOMdec + sOUT%CFLUX%MOM_to_QOM
		   if(ck_out .gt. sINP%CPOOL%MOM) then
		   sOUT%CFLUX%MOMdec=sINP%CPOOL%MOM * sOUT%CFLUX%MOMdec / ck_out
		   sOUT%CFLUX%MOM_to_QOM=sINP%CPOOL%MOM * sOUT%CFLUX%MOM_to_QOM / ck_out
         !  write(*,*) 'callme'
           end if
		   !=====================================================================================
		   !QOM
		   ck_out=sOUT % CFLUX % QOM_to_MOM + sOUT % CFLUX % QOM_to_DOM
		   if(ck_out .gt. sINP%CPOOL%QOM) then
		   sOUT % CFLUX % QOM_to_MOM = sINP%CPOOL%QOM * sOUT % CFLUX % QOM_to_MOM / ck_out
		   sOUT % CFLUX % QOM_to_DOM = sINP%CPOOL%QOM * sOUT % CFLUX % QOM_to_DOM / ck_out
         !  write(*,*) 'callme'
           end if
		   !=====================================================================================
		   !ENZP(1)
		   ck_out = sOUT % CFLUX % ENZP_to_DOM(1)
		   if(ck_out .gt. sINP % CPOOL % ENZP(1)) then
		   sOUT % CFLUX % ENZP_to_DOM(1) = sINP % CPOOL % ENZP(1)
         !  write(*,*) 'callme'
		   end if
		   !ENZP(2)
		   ck_out = sOUT % CFLUX % ENZP_to_DOM(2)
		   if(ck_out .gt. sINP % CPOOL % ENZP(2)) then
		   sOUT % CFLUX % ENZP_to_DOM(2) = sINP % CPOOL % ENZP(2)
         !  write(*,*) 'callme'
		   end if
		   !ENZM
		   ck_out = sOUT % CFLUX % ENZM_to_DOM
		   if(ck_out .gt. sINP % CPOOL % ENZM) then
		   sOUT % CFLUX % ENZM_to_DOM = sINP % CPOOL % ENZM
         !  write(*,*) 'callme'
           end if
		   !=====================================================================================
		   !DOM
		   ck_out = sOUT % CFLUX % DOM_to_MBA + sOUT % CFLUX % DOM_to_QOM
		   if(ck_out .gt. sOUT % CPOOL % DOMT) then
		   sOUT % CFLUX % DOM_to_MBA = sOUT % CPOOL % DOMT * sOUT % CFLUX % DOM_to_MBA / ck_out
		   sOUT % CFLUX % DOM_to_QOM = sOUT % CPOOL % DOMT * sOUT % CFLUX % DOM_to_QOM / ck_out
         !  write(*,*) 'callme'
           end if
           sOUT % CFLUX % DOM_to_QOM_net = sOUT % CFLUX % DOM_to_QOM - sOUT % CFLUX % QOM_to_DOM
           sOUT % CFLUX % MOM_to_QOM_net = sOUT % CFLUX % MOM_to_QOM - sOUT % CFLUX % QOM_to_MOM
		   !=====================================================================================
		   !MBA
		   ck_out = sOUT % CFLUX % MBA_mortality + sum(sOUT % CFLUX % MBA_to_ENZP) + sOUT % CFLUX % MBA_to_ENZM &
		   & + sOUT % CFLUX % MBA_to_MBD + sOUT % MNFLUX % CO2_growth + sOUT % MNFLUX % CO2_maintn_MBA
		   if(ck_out .gt. (sINP % CPOOL % MBA+ sOUT % CFLUX % DOM_to_MBA)) then
		   sOUT % CFLUX % MBA_mortality = (sINP % CPOOL % MBA+ sOUT % CFLUX % DOM_to_MBA) * sOUT % CFLUX % MBA_mortality / ck_out
		   sOUT % CFLUX % MBA_to_ENZP(1) =(sINP % CPOOL % MBA+ sOUT % CFLUX % DOM_to_MBA) * sOUT % CFLUX % MBA_to_ENZP(1) / ck_out
		   sOUT % CFLUX % MBA_to_ENZP(2) =(sINP % CPOOL % MBA+ sOUT % CFLUX % DOM_to_MBA) * sOUT % CFLUX % MBA_to_ENZP(2) / ck_out
		   sOUT % CFLUX % MBA_to_ENZM = (sINP % CPOOL % MBA+ sOUT % CFLUX % DOM_to_MBA) * sOUT % CFLUX % MBA_to_ENZM / ck_out
		   sOUT % CFLUX % MBA_to_MBD = (sINP % CPOOL % MBA+ sOUT % CFLUX % DOM_to_MBA) * sOUT % CFLUX % MBA_to_MBD / ck_out
		   sOUT % MNFLUX % CO2_growth = (sINP % CPOOL % MBA+ sOUT % CFLUX % DOM_to_MBA) * sOUT % MNFLUX % CO2_growth / ck_out
		   sOUT % MNFLUX % CO2_maintn_MBA = (sINP % CPOOL % MBA+ sOUT % CFLUX % DOM_to_MBA) * sOUT % MNFLUX % CO2_maintn_MBA / ck_out
           sOUT % CFLUX % MBA_PM = sOUT % CFLUX % MBA_mortality + sum(sOUT % CFLUX % MBA_to_ENZP) + sOUT % CFLUX % MBA_to_ENZM 
           sOUT % CFLUX % MBA_to_DOM   = sPAR % frMB2DOM * sOUT % CFLUX % MBA_mortality          
            sOUT % CFLUX % MBA_to_POM   = (1.d0 - sPAR % frMB2DOM) * sOUT % CFLUX % MBA_mortality * frMBA_to_POM  
           sOUT % MNFLUX % CO2_maintn = sOUT % MNFLUX % CO2_maintn_MBA + sOUT % MNFLUX % CO2_maintn_MBD
            sOUT % MNFLUX % CO2_gm = sOUT%MNFLUX%CO2_growth + sOUT%MNFLUX%CO2_maintn 
            sOUT % MNFLUX % CO2_gmo = sOUT % MNFLUX % CO2_gm
         !  write(*,*) 'callme'
           end if
		   !=====================================================================================
		   !MBD
		   ck_out = sOUT % MNFLUX % CO2_maintn_MBD + sOUT % CFLUX % MBD_to_MBA
		   if(ck_out .gt. sINP % CPOOL % MBD) then
		   sOUT % MNFLUX % CO2_maintn_MBD = sINP % CPOOL % MBD * sOUT % MNFLUX % CO2_maintn_MBD / ck_out
		   sOUT % CFLUX % MBD_to_MBA = sINP % CPOOL % MBD * sOUT % MNFLUX % CO2_maintn_MBD / ck_out
           sOUT % MNFLUX % CO2_maintn = sOUT % MNFLUX % CO2_maintn_MBA + sOUT % MNFLUX % CO2_maintn_MBD
            sOUT % MNFLUX % CO2_gm = sOUT%MNFLUX%CO2_growth + sOUT%MNFLUX%CO2_maintn 
            sOUT % MNFLUX % CO2_gmo = sOUT % MNFLUX % CO2_gm
         !  write(*,*) 'callme'
        ...
        IF(sINI%iKinetics.le.10) THEN
            ...
            if(sOUT % CPOOL % MBA.lt.0.D0) then  
        ...
        IF(.NOT.sINI%Carbon_only) THEN
            ...
            phi = fNLimit_MB(sINP%CN%MBA,sINI%CN_MB_min,sINI%CN_MB_max,sPAR%wNUI)       
            sPAR%YgN        = phi  !![5/25/2017]    
            ...
            rtp1 = 1.d0 + sINP%MNPOOL%NH4/sPAR%KsNH4_MB + sINP%MNPOOL%NO3/sPAR%KsNO3_MB &      
                + sINP%MNPOOL%NH4/sPAR%KsNH4_VG + sINP%MNPOOL%NO3/sPAR%KsNO3_VG         
            !! 2/24/2019: sPAR%VNup_MB -> sPAR%VNup_MB * phi
            !    sPAR%VNup_MB    = sPAR%VNup_MB * phi  
		   ...
		   !==========================================================================
		   !POM(1)
           ck_out= sOUT%NFLUX%POMdec_to_MOM(1)+sOUT%NFLUX%POMdec_to_DOM(1)
           if(ck_out .gt. sOUT % NPOOL % POM(1)) then
		   sOUT%NFLUX%POMdec_to_MOM(1)=sOUT % NPOOL % POM(1)*sOUT%NFLUX%POMdec_to_MOM(1)/ck_out
		   sOUT%NFLUX%POMdec_to_DOM(1)=sOUT % NPOOL % POM(1)*sOUT%NFLUX%POMdec_to_DOM(1)/ck_out
           sOUT%NFLUX%POMdec(1)= sOUT%NFLUX%POMdec_to_MOM(1)+ sOUT%NFLUX%POMdec_to_DOM(1)
           !write(*,*) 'callme'
		   end if
		   !POM(2)
		   ck_out= sOUT%NFLUX%POMdec_to_MOM(2)+sOUT%NFLUX%POMdec_to_DOM(2)
           if(ck_out .gt. sOUT % NPOOL % POM(2)) then
		   sOUT%NFLUX%POMdec_to_MOM(2)=sOUT % NPOOL % POM(2)*sOUT%NFLUX%POMdec_to_MOM(2)/ck_out
		   sOUT%NFLUX%POMdec_to_DOM(2)=sOUT % NPOOL % POM(2)*sOUT%NFLUX%POMdec_to_DOM(2)/ck_out
           sOUT%NFLUX%POMdec(2)= sOUT%NFLUX%POMdec_to_MOM(2)+ sOUT%NFLUX%POMdec_to_DOM(2)
           !write(*,*) 'callme1'
           end if
		   !=====================================================================================
		   !MOM
		   ck_out=sOUT%NFLUX%MOMdec + sOUT%NFLUX%MOM_to_QOM
		   if(ck_out .gt. sINP%NPOOL%MOM) then
		   sOUT%NFLUX%MOMdec=sINP%NPOOL%MOM * sOUT%NFLUX%MOMdec / ck_out
		   sOUT%NFLUX%MOM_to_QOM=sINP%NPOOL%MOM * sOUT%NFLUX%MOM_to_QOM / ck_out
           !write(*,*) 'callme2'
           end if
		   !=====================================================================================
		   !QOM
		   ck_out=sOUT % NFLUX % QOM_to_MOM + sOUT % NFLUX % QOM_to_DOM
		   if(ck_out .gt. sINP%NPOOL%QOM) then
		   sOUT % NFLUX % QOM_to_MOM = sINP%NPOOL%QOM * sOUT % NFLUX % QOM_to_MOM / ck_out
		   sOUT % NFLUX % QOM_to_DOM = sINP%NPOOL%QOM * sOUT % NFLUX % QOM_to_DOM / ck_out
           !write(*,*) 'callme3'
           end if
		   !=====================================================================================
		   !ENZP(1)
		   ck_out = sOUT % NFLUX % ENZP_to_DOM(1)
		   if(ck_out .gt. sINP % NPOOL % ENZP(1)) then
		   sOUT % NFLUX % ENZP_to_DOM(1) = sINP % NPOOL % ENZP(1)
           !write(*,*) 'callme4'
		   end if
		   !ENZP(2)
		   ck_out = sOUT % NFLUX % ENZP_to_DOM(2)
		   if(ck_out .gt. sINP % NPOOL % ENZP(2)) then
		   sOUT % NFLUX % ENZP_to_DOM(2) = sINP % NPOOL % ENZP(2)
           !write(*,*) 'callme5'
		   end if
		   !ENZM
		   ck_out = sOUT % NFLUX % ENZM_to_DOM
		   if(ck_out .gt. sINP % NPOOL % ENZM) then
		   sOUT % NFLUX % ENZM_to_DOM = sINP % NPOOL % ENZM
           !write(*,*) 'callme6'
           end if
		   !=====================================================================================
		   !DOM
		   ck_out = sOUT % NFLUX % DOM_to_MBA + sOUT % NFLUX % DOM_to_QOM
		   if(ck_out .gt. sINP % NPOOL % DOMT) then
		   sOUT % NFLUX % DOM_to_MBA = sINP % NPOOL % DOMT * sOUT % NFLUX % DOM_to_MBA / ck_out
		   sOUT % NFLUX % DOM_to_QOM = sINP % NPOOL % DOMT * sOUT % NFLUX % DOM_to_QOM / ck_out
           !write(*,*) 'callme7'
           end if
           sOUT % NFLUX % DOM_to_QOM_net = sOUT % NFLUX % DOM_to_QOM - sOUT % NFLUX % QOM_to_DOM
           sOUT % NFLUX % MOM_to_QOM_net = sOUT % NFLUX % MOM_to_QOM - sOUT % NFLUX % QOM_to_MOM
		   !=====================================================================================
		   !MBA
		   ck_out = sOUT % NFLUX % MBA_mortality + sum(sOUT % NFLUX % MBA_to_ENZP) + sOUT % NFLUX % MBA_to_ENZM &
		   & + sOUT % NFLUX % MBA_to_MBD + sOUT%MNFLUX%Nmn_MBA + 0.000000000000000000001
		   if(ck_out .gt.(sINP % NPOOL % MBA+ sOUT % NFLUX % DOM_to_MBA)) then
		   sOUT % NFLUX % MBA_mortality = (sINP % NPOOL % MBA+ sOUT % NFLUX % DOM_to_MBA) * sOUT % NFLUX % MBA_mortality / ck_out
		   sOUT % NFLUX % MBA_to_ENZP(1) = (sINP % NPOOL % MBA+ sOUT % NFLUX % DOM_to_MBA) * sOUT % NFLUX % MBA_to_ENZP(1) / ck_out
		   sOUT % NFLUX % MBA_to_ENZP(2) = (sINP % NPOOL % MBA+ sOUT % NFLUX % DOM_to_MBA) * sOUT % NFLUX % MBA_to_ENZP(2) / ck_out
		   sOUT % NFLUX % MBA_to_ENZM = (sINP % NPOOL % MBA+ sOUT % NFLUX % DOM_to_MBA) * sOUT % NFLUX % MBA_to_ENZM / ck_out
		   sOUT % NFLUX % MBA_to_MBD = (sINP % NPOOL % MBA+ sOUT % NFLUX % DOM_to_MBA) * sOUT % NFLUX % MBA_to_MBD / ck_out
		   sOUT % MNFLUX % Nmn_MBA = (sINP % NPOOL % MBA+ sOUT % NFLUX % DOM_to_MBA) * sOUT % MNFLUX% Nmn_MBA / ck_out
           sOUT % NFLUX % MBA_PM = sOUT % NFLUX % MBA_mortality + sum(sOUT % NFLUX % MBA_to_ENZP) + sOUT % NFLUX % MBA_to_ENZM 
           sOUT % NFLUX % MBA_to_DOM   = sPAR % frMB2DOM * sOUT % NFLUX % MBA_mortality         
           sOUT % NFLUX % MBA_to_POM   = (1.d0 - sPAR % frMB2DOM) * sOUT % NFLUX % MBA_mortality * frMBA_to_POM  
            
           !write(*,*) 'callme8'
           end if
		   !=====================================================================================
		   !MBD
		   ck_out = sOUT % NFLUX % MBD_to_MBA
		   if(ck_out .gt. sINP % NPOOL % MBD) then
		   sOUT % NFLUX % MBD_to_MBA = sINP % NPOOL % MBD 
           !write(*,*) 'callme9'
           end if
		   !=====================================================================================
		   !
		   !NH4
		   ck_out = sOUT%MNFLUX%Nim_NH4 + sOUT%MNFLUX%Nim_NH4_VG
		   if(ck_out .gt. sINP%MNPOOL%NH4 ) then
		   sOUT%MNFLUX%Nim_NH4 = sINP%MNPOOL%NH4 * sOUT%MNFLUX%Nim_NH4 / ck_out
		   sOUT%MNFLUX%Nim_NH4_VG = sINP%MNPOOL%NH4 * sOUT%MNFLUX%Nim_NH4_VG / ck_out
           !write(*,*) 'callme10'
           end if
		   !NO3
		   ck_out = sOUT%MNFLUX%Nim_NO3 + sOUT%MNFLUX%Nim_NO3_VG
		   if(ck_out .gt. sINP%MNPOOL%NO3 ) then
		   sOUT%MNFLUX%Nim_NO3 = sINP%MNPOOL%NO3 * sOUT%MNFLUX%Nim_NO3 / ck_out
		   sOUT%MNFLUX%Nim_NO3_VG = sINP%MNPOOL%NO3 * sOUT%MNFLUX%Nim_NO3_VG / ck_out
           !write(*,*) 'callme11'
           end if
           sOUT%MNFLUX%Nim     = sOUT%MNFLUX%Nim_NH4 + sOUT%MNFLUX%Nim_NO3
           sOUT%MNFLUX%Nim_VG     = sOUT%MNFLUX%Nim_NH4_VG + sOUT%MNFLUX%Nim_NO3_VG
           ...
           !###=========================================================================== 
            ...
            !        end if
            ELSE IF(sINI%iKinetics.eq.11) THEN
                sOUT % NPOOL % MOM  = sINP % NPOOL % MOM + sum(sOUT % NFLUX % POMdec_to_MOM) - &
                    sOUT%NFLUX%MOMdec + sOUT % NFLUX % DOMadd
            END IF!!IF(sINI%iKinetics.lt.10)
            ...
            if(dabs(sOUT%NPOOL%TM_err).gt.1.D-8) then      
                ...
                sINI%iError = -12
           ! write (1010,'(5F30.20)') sOUT%NPOOL%TM_err, sOUT%NPOOL%TM, sINP%NPOOL%TM, sOUT%NFLUX%TOTinp,sOUT%NFLUX%TOTout 
           !  write(1010,*) xx
            ...
            !!### CHECK MBC:MBN                   
            !!### (1) MBD
            sOUT%MNFLUX%CO2_ovflow_MBD_N = 0.d0
            ...
            if(rtp1.lt.sINI%CN_MB_min) then  !!C limited, N mineralization     
                ...
                sOUT%MNFLUX%CO2_ovflow_MBD_N = sOUT%CPOOL%MBD - rtp2 
                sOUT%CPOOL%MBD = rtp2                           
            end if
    
            !!### (2) MBA: STEP1                   
            sOUT%MNFLUX%CO2_ovflow_MBA_N = 0.d0
            !    sOUT%MNFLUX%Nmn_MBA = 0.d0
            if (sOUT % NPOOL % MBA .le. 0) then            
                rtp1 = sINP % CN0 % MBA                    
            else
            ...
            !!### IMMOBILIZATION: SECOND TIME
                if(rtp3.le.rtp4) then !!deficit <= availability       
                    ...
                    sOUT%MNFLUX%CO2_ovflow_MBA_N = sOUT%CPOOL%MBA - sOUT%NPOOL%MBA*sINI%CN_MB_max   
            ...
            sOUT % MNFLUX % CO2_ovflow_N  = sOUT % MNFLUX % CO2_ovflow_MBA_N + sOUT % MNFLUX % CO2_ovflow_MBD_N     
            sOUT % MNFLUX % CO2_gmo = sOUT % MNFLUX % CO2_gm + sOUT % MNFLUX % CO2_ovflow_N                     
            sOUT % MNPOOL % CO2     = sINP % MNPOOL % CO2 + sOUT % MNFLUX % CO2_gmo                           
     ...
     !     ! write(*,*) 'callme12'
        ...
        !###=====================================================================================   
                      !!!!!!!!!!!!!!!!!!!!!!!RECALCULATE INORGANIC N!!!!!!!!!!!!!!!!!!!!!!!!
		   !=====================================================================================
		   !NH4
		   ck_out = sOUT%MNFLUX%Nitrif
		   if(ck_out .gt. sOUT%MNPOOL%NH4 ) then
		   sOUT%MNFLUX%Nitrif = sOUT%MNPOOL%NH4
         !  write(*,*) 'callme14'
		   end if
		   !NO3
		   ck_out = sOUT%MNFLUX%Denit(1)
		   if(ck_out .gt. sOUT%MNPOOL%NO3 ) then
		   sOUT%MNFLUX%Denit(1) = sOUT%MNPOOL%NO3
         !  write(*,*) 'callme15'
		   end if
		   !NO2
		   ck_out = sOUT%MNFLUX%Denit(2)
		   if(ck_out .gt. sINP % MNPOOL % NO2 ) then
		   sOUT%MNFLUX%Denit(2) = sINP % MNPOOL % NO2
         !  write(*,*) 'callme16'
		   end if
		   !NO
		   ck_out = sOUT%MNFLUX%Denit(3)
		   if(ck_out .gt.  sINP % MNPOOL % NO ) then
		   sOUT%MNFLUX%Denit(3) = sINP % MNPOOL % NO
         !  write(*,*) 'callme17'
		   end if
		   !N2O
		   ck_out = sOUT%MNFLUX%Denit(4)
		   if(ck_out .gt. sINP % MNPOOL % N2O ) then
		   sOUT%MNFLUX%Denit(4) = sINP % MNPOOL % N2O
         !  write(*,*) 'callme18'
		   end if
		   !N2
		   ck_out = sOUT%MNFLUX%NFix
		   if(ck_out .gt. sINP % MNPOOL % N2 ) then
		   sOUT%MNFLUX%NFix =  sINP % MNPOOL % N2
         !  write(*,*) 'callme19'
           end if
           !===================================================================================== 
		   !=====================================================================================
            ...
            sOUT % MNPOOL % NO3     = sOUT % MNPOOL % NO3 + sOUT%MNFLUX%Nitrif * (1.d0 - phi*sPAR%rN2O ) - sOUT%MNFLUX%Denit(1)         
            sOUT%MNFLUX%Nitrifnet  =  sOUT%MNFLUX%Nitrif * (1.d0 - phi*sPAR%rN2O ) - sOUT%MNFLUX%Denit(1)
            ...
            sOUT % MNPOOL % N2O     = sINP % MNPOOL % N2O  + sOUT%MNFLUX%Nitrif*phi*sPAR%rN2O + sOUT%MNFLUX%Denit(3)&
			- sOUT%MNFLUX%Denit(4)         
...
!            rtp2 = (sINP%MNPOOL%Nmine_Free - sINP%MNPOOL%N2)/sINP%NPOOL%SOM
         ...
         !###=========================================================================== 
		   ...
		   !enzymeC1
		   ck_out = sOUT%CFLUX%ENZNm_to_DOM(1)
		   if(ck_out .gt. sINP%CPOOL%ENZNm(1) ) then
		   sOUT%CFLUX%ENZNm_to_DOM(1) = sINP%CPOOL%ENZNm(1)
         !  write(*,*) 'callme20'
		   end if 
		   !enzymeC2
		   ck_out = sOUT%CFLUX%ENZNm_to_DOM(2)
		   if(ck_out .gt. sINP%CPOOL%ENZNm(2) ) then
		   sOUT%CFLUX%ENZNm_to_DOM(2) = sINP%CPOOL%ENZNm(2)
         !  write(*,*) 'callme21'
		   end if 
		   !enzymeC3
		   ck_out = sOUT%CFLUX%ENZNm_to_DOM(3)
		   if(ck_out .gt. sINP%CPOOL%ENZNm(3) ) then
		   sOUT%CFLUX%ENZNm_to_DOM(3) = sINP%CPOOL%ENZNm(3)
         !  write(*,*) 'callme22'
		   end if 
		   !enzymeC4
		   ck_out = sOUT%CFLUX%ENZNm_to_DOM(4)
		   if(ck_out .gt. sINP%CPOOL%ENZNm(4) ) then
		   sOUT%CFLUX%ENZNm_to_DOM(4) = sINP%CPOOL%ENZNm(4)
         !  write(*,*) 'callme23'
		   end if 
		   !enzymeC5
		   ck_out = sOUT%CFLUX%ENZNm_to_DOM(5)
		   if(ck_out .gt. sINP%CPOOL%ENZNm(5) ) then
		   sOUT%CFLUX%ENZNm_to_DOM(5) = sINP%CPOOL%ENZNm(5)
         !  write(*,*) 'callme24'
		   end if 
		   !enzymeC6
		   ck_out = sOUT%CFLUX%ENZNm_to_DOM(6)
		   if(ck_out .gt. sINP%CPOOL%ENZNm(6) ) then
		   sOUT%CFLUX%ENZNm_to_DOM(6) = sINP%CPOOL%ENZNm(6)
         !  write(*,*) 'callme25'
           end if 
            !=========================================================================== 
            
		   !MBAC
		   ck_out = sum(sOUT%CFLUX%MBA_to_ENZNm)
		   if(ck_out .gt. sOUT%CPOOL%MBA ) then
		   sOUT%CFLUX%MBA_to_ENZNm(1) = sOUT%CPOOL%MBA * sOUT%CFLUX%MBA_to_ENZNm(1) / ck_out
		   sOUT%CFLUX%MBA_to_ENZNm(2) = sOUT%CPOOL%MBA * sOUT%CFLUX%MBA_to_ENZNm(2) / ck_out
		   sOUT%CFLUX%MBA_to_ENZNm(3) = sOUT%CPOOL%MBA * sOUT%CFLUX%MBA_to_ENZNm(3) / ck_out
		   sOUT%CFLUX%MBA_to_ENZNm(4) = sOUT%CPOOL%MBA * sOUT%CFLUX%MBA_to_ENZNm(4) / ck_out
		   sOUT%CFLUX%MBA_to_ENZNm(5) = sOUT%CPOOL%MBA * sOUT%CFLUX%MBA_to_ENZNm(5) / ck_out
		   sOUT%CFLUX%MBA_to_ENZNm(6) = sOUT%CPOOL%MBA * sOUT%CFLUX%MBA_to_ENZNm(6) / ck_out
         !  write(*,*) 'callme26'
           end if 
           sOUT%NFLUX%MBA_to_ENZNm =   sOUT%CFLUX%MBA_to_ENZNm/sINI%CN_ENZM
           sOUT%NFLUX%ENZNm_to_DOM =   sOUT%CFLUX%ENZNm_to_DOM/sINI%CN_ENZM
		   !!MBAN
		   !ck_out = sum(sOUT%NFLUX%MBA_to_ENZNm)
		   !if(ck_out .gt. sOUT%NPOOL%MBA ) then
		   !sOUT%NFLUX%MBA_to_ENZNm(1) = sOUT%NPOOL%MBA * sOUT%NFLUX%MBA_to_ENZNm(1) / ck_out
		   !sOUT%NFLUX%MBA_to_ENZNm(2) = sOUT%NPOOL%MBA * sOUT%NFLUX%MBA_to_ENZNm(2) / ck_out
		   !sOUT%NFLUX%MBA_to_ENZNm(3) = sOUT%NPOOL%MBA * sOUT%NFLUX%MBA_to_ENZNm(3) / ck_out
		   !sOUT%NFLUX%MBA_to_ENZNm(4) = sOUT%NPOOL%MBA * sOUT%NFLUX%MBA_to_ENZNm(4) / ck_out
		   !sOUT%NFLUX%MBA_to_ENZNm(5) = sOUT%NPOOL%MBA * sOUT%NFLUX%MBA_to_ENZNm(5) / ck_out
		   !sOUT%NFLUX%MBA_to_ENZNm(6) = sOUT%NPOOL%MBA * sOUT%NFLUX%MBA_to_ENZNm(6) / ck_out
     !   !   write(*,*) 'callme27'
         !  end if 
        !###=========================================================================== 
        !###=========================================================================== 
		   !===========================================================================
            ...
            !!### MBA and DOM mass balance (6/17/2020: for !#### bintracellular enzymes, sOUT%CFLUX%ENZNm_to_DOM is added to MBA, instead of DOM)
...
!            sOUT%NPOOL%DOMT = sOUT%NPOOL%DOMT + sum(sOUT%NFLUX%ENZNm_to_DOM)
        ...
        !###=========================================================================== 
		   ...
		   !NO
		   ck_out = sOUT%MNFLUX%NO_efflux
		   if(ck_out .gt. sOUT%MNPOOL%NO ) then
		   sOUT%MNFLUX%NO_efflux = sOUT%MNPOOL%NO
         !  write(*,*) 'callme28'
           end if 
         !  if(ck_out .lt. 0 ) then
		 !  sOUT%MNFLUX%NO_efflux = 0
         !  write(*,*) 'callme'
		!   end if
		   !N20
		   ck_out = sOUT%MNFLUX%N2O_efflux
		   if(ck_out .gt. sOUT%MNPOOL%N2O ) then
		   sOUT%MNFLUX%N2O_efflux = sOUT%MNPOOL%N2O
         !  write(*,*) 'callme29'
           end if 
     !      if(ck_out .lt. 0 ) then
		   !sOUT%MNFLUX%N2O_efflux = 0
     !    !  write(*,*) 'callme'
		   !end if
		   !N2
		   ck_out = sOUT%MNFLUX%N2_efflux
		   if(ck_out .gt. sOUT%MNPOOL%N2 ) then
		   sOUT%MNFLUX%N2_efflux = sOUT%MNPOOL%N2
         !  write(*,*) 'callme30'
           end if
     !      if(ck_out .lt. 0 ) then
		   !sOUT%MNFLUX%N2_efflux = 0
     !    !  write(*,*) 'callme'
		   !end if
          ...
          !===========================================================================   
		   ...
		   !NO3
		   ck_out = sOUT%MNFLUX%NO3_Leaching
		   if(ck_out .gt. sOUT % MNPOOL % NO3 ) then
		   sOUT%MNFLUX%NO3_Leaching = sOUT % MNPOOL % NO3
         !  write(*,*) 'callme31'
		   end if 
		   !NO2
		   ck_out = sOUT%MNFLUX%NO2_Leaching
		   if(ck_out .gt. sOUT % MNPOOL % NO2 ) then
		   sOUT%MNFLUX%NO2_Leaching = sOUT % MNPOOL % NO2
         !  write(*,*) 'callme32'
           end if 
           sOUT%MNFLUX%NO32_Leaching = sOUT%MNFLUX%NO3_Leaching + sOUT%MNFLUX%NO2_Leaching
		  !===========================================================================
		  !===========================================================================  
            ...
            CALL subMEND_NPOOL_UPDATE1(sOUT%NPOOL, sOUT%MNPOOL)     
           ! write(*,*)  sOUT%CPOOL%ENZD , sOUT% CPOOL % ENZNmt , sOUT% CPOOL % PTASE
           ! write(*,*)  sOUT%NPOOL%ENZD , sOUT% NPOOL % ENZNmt , sOUT% NPOOL % PTASE
           ! write(*,*)  sOUT%CPOOL%ENZD/sOUT%NPOOL%ENZD , sOUT% CPOOL % ENZNmt/sOUT% NPOOL % ENZNmt , sOUT% CPOOL % PTASE/sOUT% NPOOL % PTASE
            !    CALL subMEND_CN_UPDATE0(sOUT % CPOOL, sOUT % NPOOL, sOUT%MNPOOL, sOUT % CN)
            CALL subMEND_CN_UPDATE(sOUT % CPOOL, sOUT % NPOOL, sOUT%MNPOOL, sINP % CN0, sOUT % CN)   
            !CALL subMEND_CP_UPDATE(sOUT % CPOOL, sOUT % PPOOL, sOUT%MNPOOL, sINP % CP0, sOUT % CP)
            !!### C MASS BALANCE CHECK                                         
            sOUT%CFLUX%TOTout = sOUT%MNFLUX%CO2_gmo
            sOUT%CPOOL%TM_err = (sOUT%CPOOL%TM - sINP%CPOOL%TM) - (sOUT%CFLUX%TOTinp - sOUT%CFLUX%TOTout)      
...
!#### 
             !!P FLUXES:BEG           
            sOUT%PFLUX%POMdec           = sOUT%CFLUX%POMdec/sINP%CP%POM       
            sOUT%PFLUX%POMdec_to_MOM    = sOUT%CFLUX%POMdec_to_MOM/sINP%CP%POM
            sOUT%PFLUX%POMdec_to_DOM    = sOUT%CFLUX%POMdec_to_DOM/sINP%CP%POM
            
            if(sINI%iKinetics.eq.10) then
            sOUT%PFLUX%MOMdec           = sOUT%CFLUX%MOMdec/sINP%CP%QOM
            sOUT%PFLUX%MOM_to_DOM       = sOUT%CFLUX%MOM_to_DOM/sINP%CP%QOM
            else
            sOUT%PFLUX%MOMdec           = sOUT%CFLUX%MOMdec/sINP%CP%MOM
            sOUT%PFLUX%MOM_to_DOM       = sOUT%CFLUX%MOM_to_DOM/sINP%CP%MOM
            end if
    
            sOUT%PFLUX%QOM_to_DOM       = sOUT%CFLUX%QOM_to_DOM/sINP%CP%QOM
            sOUT%PFLUX%DOM_to_QOM       = sOUT%CFLUX%DOM_to_QOM/sINP%CP%DOM
            sOUT%PFLUX%DOM_to_QOM_net   = sOUT%PFLUX%DOM_to_QOM - sOUT%PFLUX%QOM_to_DOM
            
            sOUT%PFLUX%QOM_to_MOM       = sOUT%CFLUX%QOM_to_MOM/sINP%CP%QOM
            sOUT%PFLUX%MOM_to_QOM       = sOUT%CFLUX%MOM_to_QOM/sINP%CP%MOM
            sOUT%PFLUX%MOM_to_QOM_net   = sOUT%PFLUX%MOM_to_QOM - sOUT%PFLUX%QOM_to_MOM
    
            sOUT%PFLUX%MBA_to_DOM       = sOUT%CFLUX%MBA_to_DOM/sINP%CP%MBA
            sOUT%PFLUX%MBA_to_POM       = sOUT%CFLUX%MBA_to_POM/sINP%CP%MBA
            sOUT%PFLUX%MBA_mortality    = sOUT%CFLUX%MBA_mortality/sINP%CP%MBA
            sOUT%PFLUX%MBA_to_ENZP      = sOUT%CFLUX%MBA_to_ENZP/sINI%CP_ENZP      
            sOUT%PFLUX%MBA_to_ENZM      = sOUT%CFLUX%MBA_to_ENZM/sINI%CP_ENZM
            sOUT%PFLUX%MBA_to_ENZNm     = sOUT%CFLUX%MBA_to_ENZNm/sINI%CP_ENZM   
            sOUT%PFLUX%MBA_PM           = sOUT%PFLUX%MBA_mortality + sum(sOUT%PFLUX%MBA_to_ENZP) &
			+ sOUT%PFLUX%MBA_to_ENZM + sum(sOUT%PFLUX%MBA_to_ENZNm)
            
            sOUT%PFLUX%ENZP_to_DOM      = sOUT%CFLUX%ENZP_to_DOM/sINI%CP_ENZP
            sOUT%PFLUX%ENZM_to_DOM      = sOUT%CFLUX%ENZM_to_DOM/sINI%CP_ENZM
            sOUT%PFLUX%ENZNm_to_DOM     = sOUT%CFLUX%ENZNm_to_DOM/sINI%CP_ENZM    
            
            sOUT%PFLUX%MBA_to_MBD       = sOUT%CFLUX%MBA_to_MBD/sINP%CP%MBA
            sOUT%PFLUX%MBD_to_MBA       = sOUT%CFLUX%MBD_to_MBA/sINP%CP%MBD
            
            sOUT%PFLUX%DOM_to_MBA       = sOUT%CFLUX%DOM_to_MBA/SINP%CP%DOM
            
 
!sOUT%PPOOL%POM       = sOUT%CPOOL%POM/SINP%CP%POM
!sOUT%PPOOL%MOM       = sOUT%CPOOL%MOM/SINP%CP%MOM

!sINP%PPOOL%MBA       = sINP%PPOOL%MBA/SINP%CP%MBA   !# TODO  
        

!   
!rtp1 = 1 + sOUT%PPOOL%POM(1) / sPAR%KsP(1)+ sOUT%PPOOL%POM(2) / sPAR%KsP(2)+ sOUT%PPOOL%MOM/ sPAR%KsP(3)
!            
!do ipom = 1, 2
!sOUT%PFLUX%POMdec_to_SP(ipom) = sPAR%VdP(ipom) * sINP%CPOOL%PTASE * sOUT%PPOOL%POM(ipom) / (sPAR%KsP(ipom) * denom)          !# TODO 
!enddo
!       sOUT%PFLUX%MOMdec_to_SP = sPAR%VdP(3) * sINP%CPOOL%PTASE * sOUT%PPOOL%MOM(3) / (sPAR%KsP(3) * denom)
!       
!       sOUT%PFLUX%POMdec=sOUT%PFLUX%POMdec_to_SP(1) + sOUT%PFLUX%POMdec_to_SP(2)
!
!sINP%POOL%SP    = sINP%MNPOOL%DIP + sINP%MNPOOL%LIP     
!sOUT%PPOOL%POM  = sINP%POOL%POM  - sOUT%PFLUX%POMdec_to_SP(1)- sOUT%PFLUX%POMdec_to_SP(2)
!sOUT%PPOOL%MOM  = sINP%POOL%POM  - sOUT%PFLUX%MOMdec_to_SP
!sOUT%PPOOL%SP   = sINP%POOL%SP   +(sOUT%PFLUX%POMdec_to_SP(1)+ sOUT%PFLUX%POMdec_to_SP(2)+ sOUT%PFLUX%MOMdec_to_SP)
!sOUT%PPOOL%DOM  = sINP%POOL%DOM  -(sOUT%PFLUX%POMdec_to_SP(1)+ sOUT%PFLUX%POMdec_to_SP(2)+ sOUT%PFLUX%MOMdec_to_SP)

!# Pmn_MBA
          phi = fNLimit_MB(sINP%CP%MBA,sINI%CP_MB_min,sINI%CP_MB_max,sPAR%wPUI)
          sPAR%YgP= phi
          sOUT%MNFLUX%Pmn_MBA = (1.D0 - sPAR%YgP)*sOUT%PFLUX%DOM_to_MBA
   
!# Pim（first time）
           rtp1 = 1.D0 + (sINP%MNPOOL%DIP)/(sPAR%KsP_amb / bulk_den) +  &
           sPAR%enz_plant_p/sPAR%KsP_plant + &   
          (sPAR%micr_bio2enz*sINP%PPOOL%MBA)/(sPAR%KsP_amb/bulk_den) + &  !# sPAR%micr_bio2enz * amb_c * 31
          (sPAR%VdP_surf/bulk_den)/(sPAR%KsP_lab/bulk_den)
           
          sOUT%MNFLUX%Pim = (sPAR%VdP_amb * phi * (sINP%MNPOOL%DIP) * sINP%PPOOL%MBA) /  &
          ((sPAR%KsP_amb/bulk_den) * rtp1)                                                                   !###
         ! sOUT%MNFLUX%Pim = (sPAR%VdP_amb * (sINP%MNPOOL%DIP) * sPAR%micr_bio2enz*sINP%PPOOL%MBA) /  &
         ! ((sPAR%KsP_amb/bulk_den) * rtp1)                                                                  !### ED2
!# Pim_VG（first time）
          rtp1 = 1.D0 + (sINP%MNPOOL%DIP)/(sPAR%KsP_plant/bulk_den) +  &
          sPAR%enz_plant_p/sPAR%KsP_plant +  &
         (sPAR%micr_bio2enz * sINP%PPOOL%MBA) / (sPAR%KsP_amb/bulk_den) + &  
         (sPAR%VdP_surf/bulk_den)/(sPAR%KsP_lab / bulk_den)
          
          sOUT%MNFLUX%Pim_VG = sPAR%VdP_plant *sINI%VNup_VG_GPPscaler * (sINP%MNPOOL%DIP) /((sPAR%KsP_plant/bulk_den) * rtp1)!###
        !sOUT%MNFLUX%Pim_VG = sPAR%VdP_plant * (sINP%MNPOOL%DIP) /((sPAR%KsP_plant/bulk_den) * rtp1)!###  ED2
!# Pim_LIP（first time）
         ! rtp1 = sPAR%KsP_lab / bulk_den * (1.0 +  &                                            
         ! sPAR%enz_plant_p/sPAR%KsP_plant  +   &
         ! sPAR%micr_bio2enz * sINP%PPOOL%MBA / (sPAR%KsP_amb/bulk_den) + &
         !(sPAR%VdP_surf/bulk_den)/(sPAR%KsP_lab/bulk_den))
         ! 
         ! sOUT%MNFLUX%Pim_LIP = rtp1 * sPAR%VdP_surf/bulk_den / (rtp1 +sINP%MNPOOL%LIP+sINP%MNPOOL%DIP)**2  
          rtp1 = 1.D0 + (sINP%MNPOOL%DIP)/(sPAR%KsP_lab/bulk_den) +  &
          sPAR%enz_plant_p/sPAR%KsP_plant +  &
         (sPAR%micr_bio2enz * sINP%PPOOL%MBA) / (sPAR%KsP_amb/bulk_den) + &  
         (sPAR%VdP_surf/bulk_den)/(sPAR%KsP_lab / bulk_den)
          
         sOUT%MNFLUX%Pim_LIP = sPAR%VdP_surf  * (sINP%MNPOOL%DIP) /((sPAR%KsP_lab/bulk_den) * rtp1)  
         !###!!!!!!!!!!!!!!!!!!!!!POMdec!!!!!!!!!!!!!!!!!!!!!!!
		   !POM(1)
           ck_out= sOUT%PFLUX%POMdec_to_MOM(1)+sOUT%PFLUX%POMdec_to_DOM(1)
           if(ck_out .gt. sOUT % PPOOL % POM(1)) then
		   sOUT%PFLUX%POMdec_to_MOM(1)=sOUT % PPOOL % POM(1)*sOUT%PFLUX%POMdec_to_MOM(1)/ck_out
		   sOUT%PFLUX%POMdec_to_DOM(1)=sOUT % PPOOL % POM(1)*sOUT%PFLUX%POMdec_to_DOM(1)/ck_out
           sOUT%PFLUX%POMdec(1)= sOUT%PFLUX%POMdec_to_MOM(1)+ sOUT%PFLUX%POMdec_to_DOM(1)
          ! write(*,*) 'callme1'
		   end if
		   !POM(2)
		   ck_out= sOUT%PFLUX%POMdec_to_MOM(2)+sOUT%PFLUX%POMdec_to_DOM(2)
           if(ck_out .gt. sOUT % PPOOL % POM(2)) then
		   sOUT%PFLUX%POMdec_to_MOM(2)=sOUT % PPOOL % POM(2)*sOUT%PFLUX%POMdec_to_MOM(2)/ck_out
		   sOUT%PFLUX%POMdec_to_DOM(2)=sOUT % PPOOL % POM(2)*sOUT%PFLUX%POMdec_to_DOM(2)/ck_out
           sOUT%PFLUX%POMdec(2)= sOUT%PFLUX%POMdec_to_MOM(2)+ sOUT%PFLUX%POMdec_to_DOM(2)
          ! write(*,*) 'callme2'
           end if
		   !=====================================================================================
		   !MOM
		   ck_out=sOUT%PFLUX%MOMdec + sOUT%PFLUX%MOM_to_QOM
		   if(ck_out .gt. sINP%PPOOL%MOM) then
		   sOUT%PFLUX%MOMdec=sINP%PPOOL%MOM * sOUT%PFLUX%MOMdec / ck_out
		   sOUT%PFLUX%MOM_to_QOM=sINP%PPOOL%MOM * sOUT%PFLUX%MOM_to_QOM / ck_out
          ! write(*,*) 'callme3'
           end if
		   !=====================================================================================
		   !QOM
		   ck_out=sOUT % PFLUX % QOM_to_MOM + sOUT % PFLUX % QOM_to_DOM
		   if(ck_out .gt. sINP%PPOOL%QOM) then
		   sOUT % PFLUX % QOM_to_MOM = sINP%PPOOL%QOM * sOUT % PFLUX % QOM_to_MOM / ck_out
		   sOUT % PFLUX % QOM_to_DOM = sINP%PPOOL%QOM * sOUT % PFLUX % QOM_to_DOM / ck_out
          ! write(*,*) 'callme4'
           end if
		   !=====================================================================================
		   !ENZP(1)
		   ck_out = sOUT % PFLUX % ENZP_to_DOM(1)
		   if(ck_out .gt. sINP % PPOOL % ENZP(1)) then
		   sOUT % PFLUX % ENZP_to_DOM(1) = sINP % PPOOL % ENZP(1)
         !  write(*,*) 'callme5'
		   end if
		   !ENZP(2)
		   ck_out = sOUT % PFLUX % ENZP_to_DOM(2)
		   if(ck_out .gt. sINP % PPOOL % ENZP(2)) then
		   sOUT % PFLUX % ENZP_to_DOM(2) = sINP % PPOOL % ENZP(2)
         !  write(*,*) 'callme6'
		   end if
		   !ENZM
		   ck_out = sOUT % PFLUX % ENZM_to_DOM
		   if(ck_out .gt. sINP % PPOOL % ENZM) then
		   sOUT % PFLUX % ENZM_to_DOM = sINP % PPOOL % ENZM
         !  write(*,*) 'callme7'
           end if
           !%ENZNm
		   ck_out = sOUT%PFLUX%ENZNm_to_DOM(1)
		   if(ck_out .gt. sINP%PPOOL%ENZNm(1) ) then
		   sOUT%PFLUX%ENZNm_to_DOM(1) = sINP%PPOOL%ENZNm(1)
         !  write(*,*) 'callme8'
           end if
		   !%ENZNm
		   ck_out = sOUT%PFLUX%ENZNm_to_DOM(2)
		   if(ck_out .gt. sINP%PPOOL%ENZNm(2) ) then
		   sOUT%PFLUX%ENZNm_to_DOM(2) = sINP%PPOOL%ENZNm(2)
         !  write(*,*) 'callme9'
           end if
		   !%ENZNm
		   ck_out = sOUT%PFLUX%ENZNm_to_DOM(3)
		   if(ck_out .gt. sINP%PPOOL%ENZNm(3) ) then
		   sOUT%PFLUX%ENZNm_to_DOM(3) = sINP%PPOOL%ENZNm(3)
         !  write(*,*) 'callme10'
           end if
		   !%ENZNm
		   ck_out = sOUT%PFLUX%ENZNm_to_DOM(4)
		   if(ck_out .gt. sINP%PPOOL%ENZNm(4) ) then
		   sOUT%PFLUX%ENZNm_to_DOM(4) = sINP%PPOOL%ENZNm(4)
         !  write(*,*) 'callme11'
           end if
		   !%ENZNm
		   ck_out = sOUT%PFLUX%ENZNm_to_DOM(5)
		   if(ck_out .gt. sINP%PPOOL%ENZNm(5) ) then
		   sOUT%PFLUX%ENZNm_to_DOM(5) = sINP%PPOOL%ENZNm(5)
         !  write(*,*) 'callme12'
           end if
		   !%ENZNm
		   ck_out = sOUT%PFLUX%ENZNm_to_DOM(6)
		   if(ck_out .gt. sINP%PPOOL%ENZNm(6) ) then
		   sOUT%PFLUX%ENZNm_to_DOM(6) = sINP%PPOOL%ENZNm(6)
         !  write(*,*) 'callme13'
           end if
		   !=====================================================================================
		   !MBA
		   ck_out = sOUT % PFLUX % MBA_mortality + sum(sOUT % PFLUX % MBA_to_ENZP) + sOUT % PFLUX % MBA_to_ENZM &
		   + sum(sOUT%PFLUX%MBA_to_ENZNm)  + sOUT % PFLUX % MBA_to_MBD + sOUT%MNFLUX%Pmn_MBA +0.000000000000000000001
		   if(ck_out .gt. (sINP % PPOOL % MBA+ sOUT % PFLUX % DOM_to_MBA)) then
		   sOUT % PFLUX % MBA_mortality = (sINP % PPOOL % MBA+ sOUT % PFLUX % DOM_to_MBA) * sOUT % PFLUX % MBA_mortality / ck_out
		   sOUT % PFLUX % MBA_to_ENZP(1) = (sINP % PPOOL % MBA+ sOUT % PFLUX % DOM_to_MBA) * sOUT % PFLUX % MBA_to_ENZP(1) / ck_out
		   sOUT % PFLUX % MBA_to_ENZP(2) = (sINP % PPOOL % MBA+ sOUT % PFLUX % DOM_to_MBA) * sOUT % PFLUX % MBA_to_ENZP(2) / ck_out
		   sOUT % PFLUX % MBA_to_ENZM = (sINP % PPOOL % MBA+ sOUT % PFLUX % DOM_to_MBA) * sOUT % PFLUX % MBA_to_ENZM / ck_out
           sOUT % PFLUX % MBA_to_ENZNm(1) =(sINP % PPOOL % MBA+ sOUT % PFLUX % DOM_to_MBA) *sOUT % PFLUX % MBA_to_ENZNm(1) / ck_out
           sOUT % PFLUX % MBA_to_ENZNm(2) =(sINP % PPOOL % MBA+ sOUT % PFLUX % DOM_to_MBA) *sOUT % PFLUX % MBA_to_ENZNm(2) / ck_out
           sOUT % PFLUX % MBA_to_ENZNm(3) =(sINP % PPOOL % MBA+ sOUT % PFLUX % DOM_to_MBA) *sOUT % PFLUX % MBA_to_ENZNm(3) / ck_out
           sOUT % PFLUX % MBA_to_ENZNm(4) =(sINP % PPOOL % MBA+ sOUT % PFLUX % DOM_to_MBA) *sOUT % PFLUX % MBA_to_ENZNm(4) / ck_out
           sOUT % PFLUX % MBA_to_ENZNm(5) =(sINP % PPOOL % MBA+ sOUT % PFLUX % DOM_to_MBA) *sOUT % PFLUX % MBA_to_ENZNm(5) / ck_out
           sOUT % PFLUX % MBA_to_ENZNm(6) =(sINP % PPOOL % MBA+ sOUT % PFLUX % DOM_to_MBA) *sOUT % PFLUX % MBA_to_ENZNm(6) / ck_out
		   sOUT % PFLUX % MBA_to_MBD = (sINP % PPOOL % MBA+ sOUT % PFLUX % DOM_to_MBA) * sOUT % PFLUX % MBA_to_MBD / ck_out
		   sOUT % MNFLUX % Pmn_MBA = (sINP % PPOOL % MBA+ sOUT % PFLUX % DOM_to_MBA) * sOUT % MNFLUX% Pmn_MBA / ck_out
           sOUT % PFLUX % MBA_PM = sOUT % PFLUX % MBA_mortality + sum(sOUT % PFLUX % MBA_to_ENZP) + sOUT % PFLUX % MBA_to_ENZM &
		   + sum(sOUT%PFLUX%MBA_to_ENZNm)
           sOUT %PFLUX % MBA_to_DOM   = sPAR % frMB2DOM * sOUT % PFLUX % MBA_mortality         
           sOUT % PFLUX % MBA_to_POM   = (1.d0 - sPAR % frMB2DOM) * sOUT % PFLUX % MBA_mortality * frMBA_to_POM  
            
         !  write(*,*) 'callme14'
           end if
		   !MBD
		   ck_out = sOUT % PFLUX % MBD_to_MBA
		   if(ck_out .gt. sINP % PPOOL % MBD) then
		   sOUT % PFLUX % MBD_to_MBA = sINP % PPOOL % MBD 
         !  write(*,*) 'callme15'
           end if
           !DOM
		   ck_out = sOUT % PFLUX % DOM_to_MBA + sOUT % PFLUX % DOM_to_QOM
		   if(ck_out .gt. sINP % PPOOL % DOMT) then
		   sOUT % PFLUX % DOM_to_MBA = sINP % PPOOL % DOMT * sOUT % PFLUX % DOM_to_MBA / ck_out
		   sOUT % PFLUX % DOM_to_QOM = sINP % PPOOL % DOMT * sOUT % PFLUX % DOM_to_QOM / ck_out
         !  write(*,*) 'callme16'
           end if
           sOUT % PFLUX % DOM_to_QOM_net = sOUT % PFLUX % DOM_to_QOM - sOUT % PFLUX % QOM_to_DOM
           sOUT % PFLUX % MOM_to_QOM_net = sOUT % PFLUX % MOM_to_QOM - sOUT % PFLUX % QOM_to_MOM
         ...
         !###            
!### BALANCE           
sOUT%PPOOL%POM      = sINP%PPOOL%POM + sOUT%PFLUX%POMadd - sOUT%PFLUX%POMdec + sOUT%PFLUX%MBA_to_POM 
IF(sINI%iKinetics.le.10) THEN
  if(sINI%iKinetics.eq.10) then
     sOUT%PPOOL%MOM = sINP%PPOOL%MOM - sOUT%PFLUX%MOM_to_QOM_net               
     sOUT%PPOOL%QOM = sINP%PPOOL%QOM + sOUT%PFLUX%DOM_to_QOM_net&              
     + sOUT%PFLUX%MOM_to_QOM_net + sum(sOUT%PFLUX%POMdec_to_MOM) - sOUT%PFLUX%MOMdec
  else !! MOM decomposition
     sOUT%PPOOL%MOM = sINP%PPOOL%MOM - sOUT%PFLUX%MOM_to_QOM_net + sum(sOUT%PFLUX%POMdec_to_MOM) - sOUT%PFLUX%MOMdec
     sOUT%PPOOL%QOM = sINP%PPOOL%QOM + sOUT%PFLUX%DOM_to_QOM_net + sOUT%PFLUX%MOM_to_QOM_net
  end if
 
 sOUT%PPOOL%DOMT = sINP%PPOOL%DOMT + sOUT%PFLUX%DOMadd& 
  &+ sum(sOUT%PFLUX%POMdec_to_DOM)&
  &+ sOUT%PFLUX%MOM_to_DOM&
  &+ sOUT%PFLUX%MBA_to_DOM&
  &+ sum(sOUT%PFLUX%ENZP_to_DOM)&
  &+ sOUT%PFLUX%ENZM_to_DOM&
  &- sOUT%PFLUX%DOM_to_MBA&
  &- sOUT%PFLUX%DOM_to_QOM_net

   sOUT%PPOOL%ENZP = sINP%PPOOL%ENZP + sOUT%PFLUX%MBA_to_ENZP - sOUT%PFLUX%ENZP_to_DOM 
   sOUT%PPOOL%ENZM = sINP%PPOOL%ENZM + sOUT%PFLUX%MBA_to_ENZM - sOUT%PFLUX%ENZM_to_DOM 
   sOUT%PPOOL%ENZNm = sINP%PPOOL%ENZNm + sOUT%PFLUX%MBA_to_ENZNm - sOUT%PFLUX%ENZNm_to_DOM 

sOUT%PPOOL%MBA = sINP%PPOOL%MBA + sOUT%PFLUX%DOM_to_MBA& 
&- sOUT%PFLUX%MBA_PM&                                    
&- sOUT%PFLUX%MBA_to_MBD+sOUT%PFLUX%MBD_to_MBA&
&+ sOUT%MNFLUX%Pim&
&- sOUT%MNFLUX%Pmn_MBA + sum(sOUT%PFLUX%ENZNm_to_DOM)    
                                                                                                    
sOUT%PPOOL%MBD = sINP%PPOOL%MBD + sOUT%PFLUX%MBA_to_MBD - sOUT%PFLUX%MBD_to_MBA       

ELSE IF(sINI%iKinetics.eq.11) THEN
sOUT%PPOOL%MOM  = sINP%PPOOL%MOM + sum(sOUT%PFLUX%POMdec_to_MOM) - sOUT%PFLUX%MOMdec + sOUT%PFLUX%DOMadd
END IF
!###=========================================================================== 
!!!!!!!!!!!!!!!!!!!!!!INORGANIC P!!!!!!!!!!!!!!!!!!!!!!!!
		   !DIP
		   ck_out = SOUT%MNFLUX%PIM + SOUT%MNFLUX%PIM_LIP + SOUT%MNFLUX%PIM_VG 
		   if(ck_out .gt. SINP%MNPOOL%DIP) then
		   SOUT%MNFLUX%PIM = SINP%MNPOOL%DIP * SOUT%MNFLUX%PIM / ck_out
		   SOUT%MNFLUX%PIM_LIP = SINP%MNPOOL%DIP * SOUT%MNFLUX%PIM_LIP / ck_out
		   SOUT%MNFLUX%PIM_VG =  SINP%MNPOOL%DIP * SOUT%MNFLUX%PIM_VG / ck_out
         !  write(*,*) 'callme17'
           end if
!==============================================================================
!###=========================================================================== 
!### CHECK
  ...
  SOUT%MNPOOL%DIP = SINP%MNPOOL%DIP - SOUT%MNFLUX%PIM - SOUT%MNFLUX%PIM_LIP - SOUT%MNFLUX%PIM_VG     
  SOUT%MNPOOL%LIP = SINP%MNPOOL%LIP + SOUT%MNFLUX%PIM_LIP + SOUT%MNFLUX%PMN_MBA+ sOUT%MNFLUX%P_dep
  SOUT%MNPOOL%OIP = SINP%MNPOOL%OIP
  SOUT%MNPOOL%SIP = SINP%MNPOOL%SIP
  SOUT%MNPOOL%PIP = SINP%MNPOOL%PIP  
  SOUT%MNPOOL%SP  = SOUT%MNPOOL%DIP + SOUT%MNPOOL%LIP  
  
    
CALL subMEND_PPOOL_UPDATE1(sOUT%PPOOL, sOUT%MNPOOL)  
    
  sOUT%PFLUX%TOTout = sOUT%MNFLUX%Pim_VG             
  sOUT%PPOOL%TM_err = (sOUT%PPOOL%TM-sINP%PPOOL%TM) - (sOUT%PFLUX%TOTinp-sOUT%PFLUX%TOTout)
    if(dabs(sOUT%PPOOL%TM_err).gt.1.D-8) then      
       sINI%iError = -15
       write(1010,'(70F30.20)') xx
    call subMEND_ERROR_MSG("1st Nitrogen Balance Check",xx, sPAR, sINI, sOUT)
    return
 ...
 !### CN RATIO CHECK
 !### (1) MBD   
 sOUT%MNFLUX%CO2_ovflow_MBD_P = 0.d0
 sOUT%MNFLUX%Pmn_MBD = 0.d0
 
            rtp1 = sOUT%CPOOL%MBD/sOUT%PPOOL%MBD  !!CP ratio of MBD   
            if(rtp1.lt.sINI%CP_MB_min) then  !!C limited, P mineralization    
              rtp2 = sOUT%CPOOL%MBD / sINI%CP_MB_min             
              sOUT%MNFLUX%Pmn_MBD = sOUT%PPOOL%MBD - rtp2      
              sOUT%PPOOL%MBD = rtp2                            
            else if(rtp1.gt.sINI%CP_MB_max) then !!P limited, overflow        
              rtp2 = sOUT%PPOOL%MBD*sINI%CP_MB_max             
              sOUT%MNFLUX%CO2_ovflow_MBD_P = sOUT%CPOOL%MBD - rtp2 
              sOUT%CPOOL%MBD = rtp2                            
           end if   
!### （2）MBA: STEP1
 sOUT%MNFLUX%CO2_ovflow_MBA_P = 0.d0
 
           if (sOUT%PPOOL%MBA .le. 0) then            
              rtp1 = sINP % CP0 % MBA                 
           else
              rtp1 = sOUT%CPOOL%MBA/sOUT%PPOOL%MBA  !!CP ratio of MBA
           end if

           if(rtp1.lt.sINI%CP_MB_min) then  !!C limited, P mineralization  
              rtp2 = sOUT%CPOOL%MBA/sINI%CP_MB_min                         
              sOUT%MNFLUX%Pmn_MBA = sOUT%MNFLUX%Pmn_MBA + (sOUT%PPOOL%MBA - rtp2)  
              sOUT%PPOOL%MBA = rtp2                                                
            else if(rtp1.gt.sINI%CP_MB_max) then !!P limited, P IMMOBILIZATION + overflow  
              rtp2 = sOUT%CPOOL%MBA/sINI%CP_MB_max                                
              rtp3 = rtp2 - sOUT%PPOOL%MBA  !!P deficit                           
        
!!###  (2) MBA: STEP2
sOUT%MNPOOL%DIP = sINP%MNPOOL%DIP - sOUT%MNFLUX%Pim - sOUT%MNFLUX%Pim_LIP - sOUT%MNFLUX%Pim_VG 
sOUT%MNPOOL%LIP = SINP%MNPOOL%LIP + SOUT%MNFLUX%PIM_LIP + sOUT%MNFLUX%Pmn_MBA + sOUT%MNFLUX%Pmn_MBD
!# 
             rtp4 = sOUT % MNPOOL % DIP 
!!### IMMOBILIZATION: SECOND TIME
            if(rtp3.le.rtp4) then !!deficit <= availability       
                    sOUT%MNFLUX%Pim = sOUT%MNFLUX%Pim+rtp3
                    sOUT%PPOOL%MBA = rtp2
            else  !!deficit > availability                        
                    sOUT%MNFLUX%Pim = sOUT%MNFLUX%Pim+rtp4
                    sOUT%PPOOL%MBA = sOUT%PPOOL%MBA + rtp4  !!uptake all Mineral P
                    sOUT%MNFLUX%CO2_ovflow_MBA_P = sOUT%CPOOL%MBA - sOUT%PPOOL%MBA*sINI%CP_MB_max    
                    sOUT%CPOOL%MBA = sOUT%PPOOL%MBA*sINI%CP_MB_max    
                end if                                                
            end if
            sOUT%MNFLUX%Pmn     = sOUT%MNFLUX%Pmn_MBD + sOUT%MNFLUX%Pmn_MBA                      
            sOUT%MNFLUX%Pmn_net = sOUT%MNFLUX%Pmn - sOUT%MNFLUX%Pim    
            
            
!!### revisit C BALANCE
        sOUT%MNFLUX%CO2_ovflow_P  = sOUT%MNFLUX%CO2_ovflow_MBA_P + sOUT%MNFLUX%CO2_ovflow_MBD_P    
        sOUT%MNFLUX%CO2_gmo = sOUT%MNFLUX%CO2_gmo + sOUT%MNFLUX%CO2_ovflow_P                       
        sOUT%MNPOOL%CO2 = sINP%MNPOOL%CO2 + sOUT%MNFLUX%CO2_gmo                            
...
!!### P BALANCE
            sOUT%MNPOOL%DIP = sINP%MNPOOL%DIP - sOUT%MNFLUX%PIM - sOUT%MNFLUX%Pim_LIP - sOUT%MNFLUX%Pim_VG
            SOUT%MNPOOL%LIP = SINP%MNPOOL%LIP + SOUT%MNFLUX%PIM_LIP + sOUT%MNFLUX%Pmn_MBA + sOUT%MNFLUX%Pmn_MBD + sOUT%MNFLUX%P_dep !#### 
!###
!### 
       rtp1 = 1 + sOUT%PPOOL%POM(1) / sPAR%KsP(1)+ sOUT%PPOOL%POM(2) / sPAR%KsP(2)+ sOUT%PPOOL%MOM/ sPAR%KsP(3)
               
       sOUT%MNFLUX%POMdec_to_LIP(1) = sPAR%VdP(1) * sINP%CPOOL%PTASE * sOUT%PPOOL%POM(1) / (sPAR%KsP(1) * rtp1)         
       sOUT%MNFLUX%POMdec_to_LIP(2) = sPAR%VdP(2) * sINP%CPOOL%PTASE * sOUT%PPOOL%POM(2) / (sPAR%KsP(2) * rtp1) 
       sOUT%MNFLUX%MOMdec_to_LIP = sPAR%VdP(3) * sINP%CPOOL%PTASE * sOUT%PPOOL%MOM / (sPAR%KsP(3) * rtp1)               
!###=========================================================================== 
       !!!!!!!!!!!!!!!!!!!!!!INORGANIC P part3!!!!!!!!!!!!!!!!!!!!!!!!
		   !POM(1)
		   ck_out = sOUT%MNFLUX%POMdec_to_LIP(1)
		   if(ck_out .gt. sOUT%PPOOL%POM(1)) then
		   sOUT%MNFLUX%POMdec_to_LIP(1) = sOUT%PPOOL%POM(1)
         !  write(*,*) 'callme19'
           end if
		   !POM(2)
		   ck_out = sOUT%MNFLUX%POMdec_to_LIP(2)
		   if(ck_out .gt. sOUT%PPOOL%POM(2)) then
		   sOUT%MNFLUX%POMdec_to_LIP(2) = sOUT%PPOOL%POM(2)
         !  write(*,*) 'callme20'
           end if
		   !MOM
		   ck_out = sOUT%MNFLUX%MOMdec_to_LIP
		   if(ck_out .gt. sOUT%PPOOL%MOM) then
		   sOUT%MNFLUX%MOMdec_to_LIP = sOUT%PPOOL%MOM
         !  write(*,*) 'callme21'
           end if
		   !=====================================================================================
!###=========================================================================== 
!###   
sOUT%PPOOL%POM(1)  = sOUT%PPOOL%POM(1)  - sOUT%MNFLUX%POMdec_to_LIP(1)
sOUT%PPOOL%POM(2)  = sOUT%PPOOL%POM(2)- sOUT%MNFLUX%POMdec_to_LIP(2)
sOUT%PPOOL%MOM  = sOUT%PPOOL%MOM  - sOUT%MNFLUX%MOMdec_to_LIP 
sOUT%MNPOOL%LIP = SOUT%MNPOOL%LIP +(sOUT%MNFLUX%POMdec_to_LIP(1)+ sOUT%MNFLUX%POMdec_to_LIP(2)+ sOUT%MNFLUX%MOMdec_to_LIP)  
!sOUT%PPOOL%DOMT = SOUT%PPOOL%DOMT   -(sOUT%MNFLUX%POMdec_to_LIP(1)+ sOUT%MNFLUX%POMdec_to_LIP(2)+ sOUT%MNFLUX%MOMdec_to_LIP)            
!###
!###=========================================================================== 
!!!!!!!!!!!!!!!!!!!!!!INORGANIC P part4!!!!!!!!!!!!!!!!!!!!!!!!
		   !PIP
		   ck_out = sOUT%MNFLUX%weath_flux
		   if(ck_out .gt. SOUT%MNPOOL%PIP) then
		   sOUT%MNFLUX%weath_flux = SOUT%MNPOOL%PIP
         !  write(*,*) 'callme22'
           end if
		   !SIP
		   ck_out = sOUT%MNFLUX%occlu_flux
		   if(ck_out .gt. sOUT%MNPOOL%SIP) then
		   sOUT%MNFLUX%occlu_flux = sOUT%MNPOOL%SIP
          ! write(*,*) 'callme23'
           end if
		   !LIP
		   ck_out = sOUT%MNFLUX%adsorp_flux
		   if(ck_out .gt. sOUT%MNPOOL%LIP) then
		   sOUT%MNFLUX%adsorp_flux = sOUT%MNPOOL%LIP
          ! write(*,*) 'callme24'
           end if
		   !SIP
		   ck_out = sOUT%MNFLUX%desorp_flux
		   if(ck_out .gt. sOUT%MNPOOL%SIP) then
		   sOUT%MNFLUX%desorp_flux = sOUT%MNPOOL%SIP
          ! write(*,*) 'callme25'
           end if
		   !=====================================================================================
!###=========================================================================== 
!###
!#
            sOUT%MNFLUX%weath_flux = (sPAR%weath_rate*SOUT%MNPOOL%PIP) / bulk_den     
            sOUT%MNPOOL%LIP = sOUT%MNPOOL%LIP + sOUT%MNFLUX%weath_flux         
            SOUT%MNPOOL%PIP = SOUT%MNPOOL%PIP - sOUT%MNFLUX%weath_flux
            
            sOUT%MNFLUX%occlu_flux = sPAR%occlu_rate * sOUT%MNPOOL%SIP                
            sOUT%MNPOOL%SIP = sOUT%MNPOOL%SIP - sOUT%MNFLUX%occlu_flux
            sOUT%MNPOOL%OIP = sOUT%MNPOOL%OIP + sOUT%MNFLUX%occlu_flux

            sOUT%MNFLUX%adsorp_flux = sPAR%p_adsorp_rate * sOUT%MNPOOL%LIP 
            sOUT%MNPOOL%LIP = sOUT%MNPOOL%LIP - sOUT%MNFLUX%adsorp_flux
            sOUT%MNPOOL%SIP = sOUT%MNPOOL%SIP + sOUT%MNFLUX%adsorp_flux                      
                                                                                  
            sOUT%MNFLUX%desorp_flux = sPAR%p_desorp_rate * sOUT%MNPOOL%SIP 
            sOUT%MNPOOL%SIP = sOUT%MNPOOL%SIP - sOUT%MNFLUX%desorp_flux
            sOUT%MNPOOL%LIP = sOUT%MNPOOL%LIP + sOUT%MNFLUX%desorp_flux
            
!#### 
             SOUT%MNPOOL%SP  = SOUT%MNPOOL%DIP + SOUT%MNPOOL%LIP 
             sOUT%MNFLUX%PO4_Leaching = SOUT%MNPOOL%SP * sINI%fPO4_Leaching
             SOUT%MNPOOL%SP = SOUT%MNPOOL%SP - sOUT%MNFLUX%PO4_Leaching
             SOUT%MNPOOL%DIP = SOUT%MNPOOL%SP * 6.D-1
             SOUT%MNPOOL%LIP= SOUT%MNPOOL%SP * 4.D-1                   !#  resolution ratio (from century model)
!#### 
!###  Total production of P-Enzymes                                  
            
            rtp2 = (sINP%MNPOOL%LIP)/sINP%PPOOL%SOM                        
            rtp1 = sPAR%fpPTASE * rtp2 * (sum(sOUT%CFLUX%MBA_to_ENZP) + sOUT%CFLUX%MBA_to_ENZM)       
    
!### P-Enzymes production and death
            sOUT%CFLUX%MBA_to_PTASE = rtp1        
            sOUT%CFLUX%PTASE_to_DOM = sPAR%rENZM * sINP%CPOOL%PTASE                 
    
            sOUT%NFLUX%MBA_to_PTASE = sOUT%CFLUX%MBA_to_PTASE/sINI%CN_ENZM          
            sOUT%NFLUX%PTASE_to_DOM = sOUT%CFLUX%PTASE_to_DOM/sINI%CN_ENZM
            sOUT%PFLUX%MBA_to_PTASE = sOUT%CFLUX%MBA_to_PTASE/sINI%CP_ENZM          
            sOUT%PFLUX%PTASE_to_DOM = sOUT%CFLUX%PTASE_to_DOM/sINI%CP_ENZM
!####
!###=========================================================================== 
            !!!!!!!!!!!!!!!!!!!!!!INORGANIC P part5!!!!!!!!!!!!!!!!!!!!!!!!
		   !PTASE
		   ck_out = sOUT%CFLUX%PTASE_to_DOM 
		   if(ck_out .gt. sINP%CPOOL%PTASE) then
		   sOUT%CFLUX%PTASE_to_DOM = sINP%CPOOL%PTASE 
         !  write(*,*) 'callme26'
           end if
		   !===================================================================
!###=========================================================================== 
!###=========================================================================== 
           !MBAC
		   ck_out = sOUT%CFLUX%MBA_to_PTASE
		   if(ck_out .gt. sOUT%CPOOL%MBA) then
		   sOUT%CFLUX%MBA_to_PTASE = sOUT%CPOOL%MBA
           sOUT%PFLUX%MBA_to_PTASE = sOUT%CFLUX%MBA_to_PTASE/sINI%CP_ENZM
           ! write(*,*) 'callme27' 
           ELSE
           sOUT%PFLUX%MBA_to_PTASE = sOUT%CFLUX%MBA_to_PTASE/sINI%CP_ENZM       
           ...
           !MBAP
		   ck_out = sOUT%PFLUX%MBA_to_PTASE
		   if(ck_out .gt. sOUT%PPOOL%MBA) then
		   sOUT%PFLUX%MBA_to_PTASE = sOUT%PPOOL%MBA
           sOUT%CFLUX%MBA_to_PTASE = sOUT%PPOOL%MBA * sINI%CP_ENZM
         !  write(*,*) 'callme28'
           end if  
     ...
     !      end if  
!### =========================================================================== 
!### P-Enzyme mass balance
            sOUT%CPOOL%PTASE = sINP%CPOOL%PTASE + sOUT%CFLUX%MBA_to_PTASE - sOUT%CFLUX%PTASE_to_DOM      
            sOUT%NPOOL%PTASE = sOUT%CPOOL%PTASE/sINI%CN_ENZM
            sOUT%PPOOL%PTASE = sOUT%CPOOL%PTASE/sINI%CP_ENZM                                             

!### MBA and DOM mass balance 
            sOUT%CPOOL%MBA = sOUT%CPOOL%MBA - sOUT%CFLUX%MBA_to_PTASE ! + sOUT%CFLUX%PTASE_to_DOM     
            sOUT%PPOOL%MBA = sOUT%PPOOL%MBA - sOUT%PFLUX%MBA_to_PTASE ! + sOUT%PFLUX%PTASE_to_DOM     
sOUT%CPOOL%PTASE = sINP%CPOOL%PTASE + sOUT%CFLUX%MBA_to_PTASE - sOUT%CFLUX%PTASE_to_DOM         
sOUT%NPOOL%PTASE = sOUT%CPOOL%PTASE/sINI%CN_ENZM
sOUT%PPOOL%PTASE = sOUT%CPOOL%PTASE/sINI%CP_ENZM     
            
            sOUT%CPOOL%DOMT = SOUT%CPOOL%DOMT + sOUT%CFLUX%PTASE_to_DOM
            sOUT%PPOOL%DOMT = SOUT%PPOOL%DOMT + sOUT%PFLUX%PTASE_to_DOM
            sOUT%CPOOL%DOM  = sOUT%CPOOL%DOMT * sINI%fDOM                          
            sOUT%CPOOL%DOMS = sOUT%CPOOL%DOMT - sOUT%CPOOL%DOM                     
            sOUT%PPOOL%DOM  = sOUT%PPOOL%DOMT * sINI%fDOM                          
            sOUT%PPOOL%DOMS = sOUT%PPOOL%DOMT - sOUT%PPOOL%DOM                     
!#### OUTPUT    write(*,*)'DOMT/DOMS/DOM=',rtp1,sOUT%CPOOL%DOMT,sOUT%CPOOL%DOMS,sOUT%CPOOL%DOM
!### revisit C BALANCE
            CALL subMEND_CPOOL_UPDATE1(sINP%CPOOL)  !! add ENZNmt   
            CALL subMEND_CPOOL_UPDATE1(sOUT%CPOOL)                  
            CALL subMEND_NPOOL_UPDATE1(sOUT%NPOOL, sOUT%MNPOOL)     
...
!### P MASS BALANCE CHECK                                        
            sOUT%PFLUX%TOTout =  sOUT%MNFLUX%Pim_VG + sOUT%MNFLUX%PO4_Leaching  
            sOUT%PPOOL%TM_err = (sOUT%PPOOL%TM-sINP%PPOOL%TM) - (sOUT%PFLUX%TOTinp-sOUT%PFLUX%TOTout)      
            if(dabs(sOUT%PPOOL%TM_err).gt.1.D-8) then        
                sINI%iError = -25
         !   write (1010,'(5F30.20)') sOUT%PPOOL%TM_err, sOUT%PPOOL%TM, sINP%PPOOL%TM, sOUT%PFLUX%TOTinp,sOUT%PFLUX%TOTout 
           write(1010,'(70F30.20)') xx
                call subMEND_ERROR_MSG("2nd Phosphorus Balance Check",xx, sPAR, sINI, sOUT)
                return
            end if
            !!P MASS BALANCE: END
        END IF !!IF(.NOT.sINI%Carbon_only)
    ...
    SUBROUTINE subMEND_PAR(xx, sPAR, sINI)
        ...
        !    USE MOD_MEND
        !    IMPLICIT NONE
        !    REAL(KIND=8), PARAMETER :: Tref = 20.d0  !!moved to MOD_MEND_TYPE
        !    REAL(KIND=8) fTArh, fT_CUE !!FUNCTIONS
        !    REAL(KIND=8) fSWP, fSWP_OPT, fSWP_A2D, fSWP_D2A !!FUNCTIONS
        !    REAL(KIND=8) fpH  !!FUNCTIONS
        ...
        REAL(KIND=8) CUE_slope,CUE_ref, SWPmin !!CUE_slope = (-0.005, -0.016)
        !    REAL(KIND=8) Vm0  !!specific maintenance rate without any modification
    
        REAL(KIND=8) tp_scalar       !!temperature scalar
        REAL(KIND=8) wp_scalar       !!INCrease with increasing wp
        REAL(KIND=8) wp_scalar_low   !!DECrease with increasing wp
        REAL(KIND=8) wp_scalar_opt   !!with increasing wp, INcrease first to OPTimal condition, then decrease
        REAL(KIND=8) pH_scalar   !!pH scalar
    
        REAL(KIND=8) tp
        REAL(KIND=8) wp, wfp !!wp=water matric potential, wfp= water-filled porosity
        REAL(KIND=8) pH
...
!        sPARbase%KsPOM(2)   = 6.0
    
        if(sINI%iKinetics.eq.10) then
            sPARbase%KsMOM      = xx(6)        !! QOM decomposition
        else
            sPARbase%KsMOM      = xx(6)*xx(7)  !! MOM decomposition
!            sPARbase%KsMOM      = xx(6)*xx(7)*0.8  !! MOM decomposition,  *0.8
        end if
    
        sPARbase%frMB2POMO  = xx(8)
        ...
        sPARbase%Vdenit(2)     = xx(70) !!NO2 -> NO
        sPARbase%Vdenit(3)     = xx(71) !!NO -> N2O xx(34)
        sPARbase%Vdenit(4)     = xx(72) !!N2O -> N2 xx(34)
		sPARbase%rN2O = xx(75) !!
        ...
        sPARbase%KsDenit(2)     = xx(73) !!NO2 -> NO
        sPARbase%KsDenit(3)     = xx(38) !!NO -> N2O
        sPARbase%KsDenit(4)     = xx(74) !!N2O -> N2
        ...
        sPARbase%wNUI      = xx(44)  !! exponent to compute nitrogen use index (NUI)
        sPAR%wNUI          = sPARbase%wNUI

        sPARbase%Qmax_NH4     = xx(45) !!NH4 adsorption capacity
        sPARbase%Kba_NH4      = xx(46) !!binding affinity
    
        sPARbase%Kp2u      = xx(47)
        sPARbase%Ku2p      = xx(48)
        sPARbase%wPUI      = xx(49)           !####   
        sPAR%wPUI          = sPARbase%wPUI    !#### 
        sPARbase%fpPTASE      = xx(50)         !#### 
        sPARbase%VdP(1)      = xx(51)         !#### 
        sPARbase%VdP(2)      = xx(51)         !#### 
        sPARbase%VdP(3)      = xx(51)         !#### 
        sPARbase%KsP(1)      = xx(54)         !#### 
        sPARbase%KsP(2)      = xx(54)         !#### 
        sPARbase%KsP(3)      = xx(54)         !#### 
        sPARbase%KsP_amb      = xx(57)        !#### 
        sPARbase%KsP_plant      = xx(58)      !#### 
        sPARbase%KsP_lab      = xx(59)        !#### 
        sPARbase%micr_bio2enz = xx(60)        !####
        sPAR%micr_bio2enz = sPARbase%micr_bio2enz    !#### 
        sPARbase%VdP_amb      = xx(61)        !#### 
        sPARbase%VdP_plant      = xx(62)      !#### 
        sPARbase%VdP_surf      = xx(63)       !#### 
        sPARbase%enz_plant_p      = xx(64)    !#### 
        sPAR%enz_plant_p = sPARbase%enz_plant_p      !#### 
        sPARbase%weath_rate      = xx(65)     !#### 
        sPARbase%occlu_rate      = xx(66)     !#### 
        sPARbase%p_adsorp_rate      = xx(67)  !#### 
        sPARbase%p_desorp_rate      = xx(68)  !#### 
        sPARbase%rPleach = xx(69) !!PO4
        
        sPAR % frMB2POMO= sPARbase % frMB2POMO
        sPAR % fINP     = sPARbase % fINP
        sPAR % Q10      = sPARbase % Q10
        sPAR % rNleach  = sPARbase % rNleach
        sPAR % rPleach  = sPARbase % rPleach
		sPAR % rN2O  = sPARbase %  rN2O
        sPAR % bNup_VG = sPARbase%bNup_VG
        sPAR % fRa        = sPARbase%fRa
        sPAR % fpENZN   = sPARbase%fpENZN
        sPAR % fpPTASE   = sPARbase%fpPTASE                        !####    
        ...
        wp_scalar = fSWP(wp, BIOME, SOM)                                                                             !####
        pH_scalar = fpH(sCase,pH)                                                                                    !####
        sPAR % VdP     = sPARbase % VdP*tp_scalar*wp_scalar*pH_scalar                                                !####
        !sPAR % VdP(2)     = sPARbase % VdP(2)*tp_scalar*wp_scalar*pH_scalar                                         !####
        !sPAR % VdP(3)     = sPARbase % VdP(3)*tp_scalar*wp_scalar*pH_scalar                                         !####
        !# REAL RATE                                                                                                 !####
        sCase = "Km"                                                                                                 !####
        tp_scalar = fTArh(sCase, tp, const_Tref)                                                                     !####
        !    if(sINI%iTmp_Func.eq.0) then                                                                            !####
        ...
        !    end if                                                                                                  !####
                                                                                                                     !####
        sPAR % KsP(1)        = sPARbase % KsP(1)*tp_scalar                                                            !####
        sPAR % KsP(2)        = sPARbase % KsP(2)*tp_scalar                                                            !####
        sPAR % KsP(3)        = sPARbase % KsP(3)*tp_scalar                                                            !####
        !                                                                                           !####
        ...
        sPAR % Kp2u         = sPARbase%Kp2u*tp_scalar     !# MOM TO QOM
        ...
        sPAR % weath_rate   = sPARbase%weath_rate*tp_scalar              !####
        sPAR % occlu_rate   = sPARbase%occlu_rate*tp_scalar              !####
        sPAR % p_adsorp_rate= sPARbase%p_adsorp_rate*tp_scalar           !####
        sPAR % p_desorp_rate   = sPARbase%p_desorp_rate*tp_scalar        !####
        !
        ...
        sPAR % VdP_amb      = sPARbase % VdP_amb*tp_scalar       !####
        !#
        sCase = "Km"
        tp_scalar = fTArh(sCase, tp, const_Tref)
        !    if(sINI%iTmp_Func.eq.0) then
        ...
        sPAR % KsP_amb     = sPARbase % KsP_amb*tp_scalar        !####
        ...
        sPAR%VdP_plant = sPARbase%VdP_plant * tp_scalar * wp_scalar           
        sPAR%VdP_surf = sPARbase%VdP_surf * tp_scalar * wp_scalar             
        !
        sPAR%KsNH4_VG = sPARbase%KsNH4_VG * tp_scalar
        sPAR%KsNO3_VG = sPARbase%KsNO3_VG * tp_scalar
        sPAR%KsP_plant = sPARbase%KsP_plant * tp_scalar                       
        sPAR%KsP_lab = sPARbase%KsP_lab * tp_scalar                           
        !
        !!-------------------------------------------
          !!NH4 ADSORPTION
    ...
    !! MODEL KINETICS
    !------------------------------------------------------------------
    REAL(KIND=8) FUNCTION fV_MM(iType, sPAR, sINP)
    ...
    END !!FUNCTION fV_MM
    !------------------------------------------------------------------
    SUBROUTINE sAdsorption(sPAR, sINP, sOUT)
        ...
        return
    END !!FUNCTION sAdsorption
    !------------------------------------------------------------------

    !!=================================================================
    !! TEMPERATURE SCALAR: BEGIN
    !------------------------------------------------------------------
    REAL(KIND=8) function fTArrhenius(tmp, Ea)
        !!ARGUMENTS:
        REAL(KIND=8)tmp,Ea
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fTArh0(T, Tref, Ea)
        !    USE MOD_MEND
        ...
        !temp (C): temperature
        !!ARGUMENTS:
        REAL(KIND=8), intent(in) :: T, Tref, Ea
        REAL(KIND=8) TKref, TK
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fTArh(sCase, T, Tref)
        !    USE MOD_MEND
        !fTvm,[-] temperature-adjusted factor for maximum reaction rate in M-M kinetics
        !fT = 1 at T = Tref
    ...
    END !!function fTArh
    !------------------------------------------------------------------
    REAL(KIND=8) function fT_Linear(T, Tref, slope, intercept)
        !fKmT: [mg/m3], half-saturation constant in M-M kinetics (Km)modified by Temperature
        !slope: [mg/m3/C]
        !intercept: [mg/m3]
        !!ARGUMENTS:
        REAL(KIND=8), intent(in) :: T, Tref, slope, intercept
    
        fT_Linear = slope * (T - Tref) + intercept
        return
    END
    !------------------------------------------------------------------
    REAL(KIND=8) function fT_CUE(T,Tref,slope,CUEref)
        !fT_CUE: [-], half-saturation constant in M-M kinetics (Km)modified by Temperature
        !slope: [1/degree C]
        !intercept: [-]
        !! parameter values: Wang et al. (2015), ISMEJ
        !!ARGUMENTS:
        REAL(KIND=8), intent(in):: T, Tref,slope,CUEref
        !    REAL(KIND=8), PARAMETER :: Tref      = 0    !![degree C]
        !    REAL(KIND=8), PARAMETER :: slope     = -0.01
        !    REAL(KIND=8), PARAMETER :: intercept = 0.56  !! []
    
        !!LOCAL VARIABLES:
        REAL(KIND=8), PARAMETER:: CUEmax = 0.9D0
        REAL(KIND=8), PARAMETER:: CUEmin = 0.1D-1
    ...
    REAL(KIND=8) function fTAG(tmp)
        ...
        !!LOCAL VARIABLES:
        REAL(KIND=8) A, S, Ha, Hdl, Hdh, b1, b2, a1, a2, a3
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fTQ10(T, Tref, Q10)
        !    USE MOD_MEND
        !fTvm,[-] temperature-adjusted factor for maximum reaction rate in M-M kinetics
        !fT = 1 at T = Tref
        !temp (C): temperature
        !!ARGUMENTS:
        REAL(KIND=8), intent(in) :: T, Tref, Q10
        fTQ10 = Q10**((T-Tref)/10.D0)
        return
    END !!function fTArh0

    !------------------------------------------------------------------
    REAL(KIND=8) function fTQ10_MEND(sCase, T, Tref,Q10_base)
        !    USE MOD_MEND
        !fTvm,[-] temperature-adjusted factor for maximum reaction rate in M-M kinetics
        !fT = 1 at T = Tref
        ...
        !!ARGUMENTS:
        CHARACTER(*), intent(in) :: sCase
        REAL(KIND=8)     , intent(in) :: T, Tref, Q10_base
    
        !!LOCAL VARIABLES:
        !    REAL(KIND=8) TKref, TK, Ea
        REAL(KIND=8) Q10, Q10_multiplier
    ...
    Subroutine subVGparameter(SoilTexture,SWCres,SWCsat,alpha,n)
        ...
        REAL(KIND=8)          , intent(out) :: SWCres,SWCsat,alpha,n
    ...
    !!-----------------------------------------------------------------
    REAL(KIND=8) function fSWP2SWC(SWP,SWP_units,SWCres,SWCsat,alpha,n)
        !!van-Genuchten equation
        !!convert SWP(cm) to SWC (0-1)
        !!SWP will be converted to cm
        !!ARGUMENTS:
        CHARACTER(len=*) SWP_units
        REAL(KIND=8),intent(in):: SWP,SWCres,SWCsat,alpha,n
    
        !!LOCAL VARIABLES:
        REAL(KIND=8) SWP_cm, m, eff_sat  !!effective saturation
    ...
    !!-----------------------------------------------------------------
    REAL(KIND=8) function fSWC2SWP(SWC0,SWCres,SWCsat,alpha,n)
        ...
        !    USE MOD_MEND
        !    IMPLICIT NONE
        REAL(KIND=8),PARAMETER::rlim = 1.01d0
        !!ARGUMENTS:
        REAL(KIND=8),intent(in):: SWC0
        REAL(KIND=8) SWCres,SWCsat,alpha,n
    
        !!LOCAL VARIABLES:
        REAL(KIND=8) SWC !!,SWPmin  !!min SWP <0
        REAL(KIND=8) m, eff_sat  !!effective saturation
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fSWP0(SWP,SWPmin,w)
        !Rate Scalar for Soil Water Potential (SWP)
        !Manzoni et al (2012), Ecology, 93: 930-938
        !    USE MOD_MEND
        !    IMPLICIT NONE
    
        REAL(KIND=8), PARAMETER :: SWP_FC = -0.033 ![MPa], field capacity SWP
        !!ARGUMENTS:
        REAL(KIND=8) SWP, SWPmin ![MPa]
        REAL(KIND=8) w
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fSWP(SWP,BIOME,SOM)
    ...
    END !!function fSWP
    !------------------------------------------------------------------
    REAL(KIND=8) FUNCTION fSWP_Death(SWP,SWPmin,w)
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fSWP_OPT(SWP)
        !SWP Scalar for SOM (lignin) decomposition
        !Hansen et al (1990), DAISY Model, page 105, Eq (6-16)
        !
        !    REAL(KIND=8), PARAMETER :: SWPmin = -dexp(4.5*dlog(10d0))   !![MPa]
        !    REAL(KIND=8), PARAMETER :: SWPlow = -dexp(0.5*dlog(10d0))
        !    REAL(KIND=8), PARAMETER :: SWPhigh= -dexp(-0.5*dlog(10d0))
        !    REAL(KIND=8), PARAMETER :: SWPmax = -dexp(-2.0*dlog(10d0))
        !    !!ARGUMENTS:
        !    REAL(KIND=8) SWP ![MPa]
        !
        ...
        REAL(KIND=8), PARAMETER :: SWPmin = -dexp(2.5*dlog(10d0))   !![MPa]
        REAL(KIND=8), PARAMETER :: SWPlow = -dexp(-1.5*dlog(10d0))
        REAL(KIND=8), PARAMETER :: SWPhigh= -dexp(-2.5*dlog(10d0))
        REAL(KIND=8), PARAMETER :: SWPmax = -dexp(-4.0*dlog(10d0))
        !!ARGUMENTS:
        REAL(KIND=8) SWP ![MPa]
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fSWP_CLM(SWP,SWPsat)
        !Rate Scalar for Soil Water Potential (SWP)
        !CLM4.5 Technical Note, page 285, Eq (15.6)
        !Andren and Paustain (1987), Orchard and Cook (1983).
    
        REAL(KIND=8), PARAMETER :: SWPmin = -10 ![MPa], lower limit for SWP control on decomposition rate
        !!ARGUMENTS:
        REAL(KIND=8) SWP, SWPsat ![MPa], saturated SWP
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fWFP_PieceWise0(WFP,WFPcr,Slope,Intercept)
        !SWP Scalar for Nitrification/Denitrification
        !Muller (1999)
    
        !!ARGUMENTS:
        REAL(KIND=8) WFP ![-], water-filled porosity
        REAL(KIND=8) WFPcr(4), Slope(2), Intercept(2)
    ...
    END !!fWFP_PieceWise0
    !------------------------------------------------------------------
    REAL(KIND=8) function fWFP_PieceWise(sCase,WFP)
        !SWP Scalar for Nitrification/Denitrification
        !Muller (1999)
    ...
    END !!fWFP_PieceWise
    !------------------------------------------------------------------
    REAL(KIND=8) function fSWPsat(p_sand,p_clay)
        ...
        REAL(KIND=8), PARAMETER :: SWP_base = -9.8d-5 ![MPa], base SWP
        REAL(KIND=8), PARAMETER :: a = 1.54d0
        REAL(KIND=8), PARAMETER :: b = 9.5d-3
        REAL(KIND=8), PARAMETER :: c = 6.3d-3
    
        !!ARGUMENTS:
        REAL(KIND=8) p_sand, p_clay, p_silt
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fSWP_A2D(SWP, SWP_A2D, w)
        !Soil Water Scalar for Microbial Dormancy
        !! Manzoni (2014) SBB, 73: 69-83

        !!ARGUMENTS:
        REAL(KIND=8), intent(in):: SWP_A2D !! = -0.4 [MPa],
        REAL(KIND=8), intent(in):: SWP
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fSWP_D2A(SWP, SWP_D2A, w)
        !!Soil Water Scalar for Microbial reactivation
        !!ARGUMENTS:
        REAL(KIND=8), intent(in):: SWP_D2A !! = 1/4*SWP_A2D [MPa]
        REAL(KIND=8), intent(in):: SWP
        REAL(KIND=8), intent(in):: w       !! = 4
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fO2_CONC(SWC,Porosity)
        !!fraction of O2 concentration in soil [m3 O2/cm3 air]
        !!Davidson et al., 2012, GCB, 18: 371-384
        !!ARGUMENTS:
        REAL(KIND=8), intent(in):: SWC
        REAL(KIND=8), intent(in):: Porosity
    
        !!LOCAL VARIABLES
        REAL(KIND=8), PARAMETER :: Dgas = 1.67D0 !!dimensionless diffusion coefficient for O2 in air
        REAL(KIND=8), PARAMETER :: frac_O2_in_air = 0.209D0
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fO2_scalar(SWC,Porosity)
        ...
        !!LOCAL VARIABLES
        REAL(KIND=8) Ks_O2 !!half-saturation constant for O2
        REAL(KIND=8) O2_CONC !!O2 CONCENTRATION, [m3 O2/cm3 air]
        !
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function fracDOM(SWC,SWCsat)
        !!fraction of DOM in SOLM (SOLuble OM)) [-]
        !!Davidson et al., 2012, GCB, 18: 371-384
        !!ARGUMENTS:
        REAL(KIND=8), intent(in):: SWC
        REAL(KIND=8), intent(in):: SWCsat !! Saturation
        !    REAL(KIND=8), intent(in):: exp_dissolve  !! default = 3
    ...
    !------------------------------------------------------------------
    REAL(KIND=8) function KSWC(SWC,SWCsat,Ksat,Lambda)
        ...
        !!ARGUMENTS:
        REAL(KIND=8), intent(in):: SWC,SWCsat,Ksat,Lambda
    ...
    END !!function KSWC
    !------------------------------------------------------------------
    !REAL(KIND=8) function fracNO3_Leaching(STP,SWC,SWCsat,SWCfc,Ksat,Lambda,rNleach,soil_depth,dt)
    REAL(KIND=8) function fracNO3_Leaching(SWC,SWCsat,SWCfc,Ksat,Lambda,rNleach,soil_depth,dt)    !# NO3淋溶的部分
        ...
        !! page 151
        !!ARGUMENTS:
        !    REAL(KIND=8), intent(in):: STP  !! soil temperature
        REAL(KIND=8), intent(in):: SWC, SWCsat,SWCfc,Ksat,Lambda,soil_depth,dt
        REAL(KIND=8), intent(in):: rNleach  !! additional coefficient, between 0 and 1
    
        !!LOCAL VARIABLES
        !    REAL(KIND=8), PARAMETER :: Dgas = 1.67D0 !!dimensionless diffusion coefficient for O2 in air
        !    REAL(KIND=8), PARAMETER :: frac_O2_in_air = 0.209D0
        REAL(KIND=8) SWCexcess  !!excess SW [cm]
        REAL(KIND=8) SWCperc    !!SWC percolating out of the study domain
        REAL(KIND=8) time_perc  !![h]
        REAL(KIND=8) frac_NO3_dissolved
        REAL(KIND=8) KSWC0
    ...
    !------------------------------------------------------------------
        !REAL(KIND=8) function fracPO4_Leaching(STP,SWC,SWCsat,SWCfc,Ksat,Lambda,rPleach,soil_depth,dt)
    REAL(KIND=8) function fracPO4_Leaching(SWC,SWCsat,SWCfc,Ksat,Lambda,rPleach,soil_depth,dt)    ! PO4leaching
        !!fraction of PO4 leaching out of the study domain [-]
        !!Neitsch, S. L., Arnold, J. G., Kiniry, J. R. & Williams, J. R. Soil and Water Assessment Tool Theoretical Documentation. Version 2009. 647 (Texas Water Resources Institute, Texas A&M University System, College Station, TX, 2011).
        !! page 151
        !!ARGUMENTS:
        !    REAL(KIND=8), intent(in):: STP  !! soil temperature
        REAL(KIND=8), intent(in):: SWC, SWCsat,SWCfc,Ksat,Lambda,soil_depth,dt
        REAL(KIND=8), intent(in):: rPleach  !! additional coefficient, between 0 and 1
        ...
        REAL(KIND=8) SWCperc    !!SWC percolating out of the study domain
        REAL(KIND=8) time_perc  !![h]
        REAL(KIND=8) frac_PO4_dissolved
        REAL(KIND=8) KSWC0
    
        KSWC0 = KSWC(SWC,SWCsat,Ksat,Lambda)
        KSWC0 = max(KSWC0,1.d-10)
    
        frac_PO4_dissolved = fracDOM(SWC,SWCsat)
        SWCexcess = max(0.d0,SWC-SWCfc)
        ...
        fracPO4_Leaching = rPleach * frac_PO4_dissolved * (SWCperc/max(SWC,1.d-4))
        !    if(STP.lt.0.d0) then !! frozen soil
        !        fracPO4_Leaching = 0.d0
        !    end if
        return
    END !!function fracPO4_Leaching
    !------------------------------------------------------------------
    REAL(KIND=8) function fNLimit_MB(MB_CN,MB_CN_min,MB_CN_max,wexp)    
        !!Nitrogen limitation level of microbes
        !!higher MB_CN, higher fNLimit_MB, higher N limitation
        !!ARGUMENTS:
        REAL(KIND=8), intent(in):: MB_CN,MB_CN_min,MB_CN_max
        REAL(KIND=8), intent(in):: wexp !!exponential, >=1
    ...
    !! pH Scalar: BEGIN
    !------------------------------------------------------------------
    REAL(KIND=8) FUNCTION fpH(sCase,pH)
        !    REAL(KIND=8) fpH0 !! function
        !!ARGUMENTS:
        CHARACTER(len=*) , intent(in) :: sCase
        REAL(KIND=8)          , intent(in) :: pH !pH value
    
        !!LOCAL VARIABLES
        REAL(KIND=8) pHopt !optimum pH
        REAL(KIND=8) pHsen !pH sensitivity
    ...
    END !!FUNCTION fpH
    !------------------------------------------------------------------
    REAL(KIND=8) FUNCTION fpH0(pH, pHopt, pHsen)
        !!ARGUMENTS:
        REAL(KIND=8), intent(in) :: pH !pH value
        REAL(KIND=8), intent(in) :: pHopt !optimum pH
        REAL(KIND=8), intent(in) :: pHsen !pH sensitivity
    ...
    !!-----------------------------------------------------------------
    REAL(KIND=8) FUNCTION fPa(altitude)
        !! Atmospheric Pressure (kPa)
        REAL(KIND=8), PARAMETER:: AltMax = 8200d0                         
    ...
    REAL(KIND=8) FUNCTION fDCair(DCa0, altitude, temp)
        !!diffusivity of air
        !!ARGUMENTS:
        REAL(KIND=8) DCa0, altitude, temp
    ...
    SUBROUTINE subDiffusivity(sPAR, sINP, sOUT)
        ...
        !!LOCAL VARIABLES:
        INTEGER i
        REAL(KIND=8) AFPPoroIn, AFPPoroEx, SWC
        REAL(KIND=8) dtmp1, dtmp2, dtmp3
    ...
    REAL(KIND=8) FUNCTION fNOxAir(Ngas, Altitude, Temperature)
        !! unit of NOxAir: g N m-3
        !!ARGUMENTS:
        CHARACTER(len=*) , intent(in) :: Ngas
        REAL(KIND=8), intent(in) :: AltiTude, Temperature
    
        !!LOCAL VARIABLES:
        INTEGER nN
        REAL(KIND=8) Pressure, VolAir, NOxAir_ppb, MolMass
    ...
    !!Gas diffusion
    REAL(KIND=8) FUNCTION fGasEfflux(Ngas,Gas_Conc_Soil,Porosity, Diff_dist,Altitude, SWCFC, SWC, Temperature, DiffAir0)
        !! mg C/cm3/h
        !!ARGUMENTS:
        CHARACTER(len=*) , intent(in) :: Ngas
        REAL(KIND=8), intent(in):: Gas_Conc_Soil,Porosity, Diff_dist, Altitude, SWC, SWCFC, Temperature, DiffAir0
        !!                       mg N/m3    cm3/cm3   ,   cm,       m        m3/m3, oC       ,m2/h
    
        !!LOCAL VARIABLES:
        REAL(KIND=8) DiffAir, DiffSoil, Gas_Conc_Air,fGasEfflux_unit_area  !!pressure,
    ...
    !! ISOTOPES: BEGIN
    !!-----------------------------------------------------------------
    REAL(KIND=8) FUNCTION fPermil(iOpt, Rstd, iso1, iso2)
        ...
        REAL(KIND=8), intent(in) :: Rstd !standard ratio of iso2/iso1, e.g., C14/C12 = 1d-12, C13/C12 = 0.0112372
        REAL(KIND=8), intent(in) :: iso1 !concentration of iso1
        REAL(KIND=8), intent(in) :: iso2 !concentration of iso2, or iso2/iso1
        if (iOpt .eq. 0) then
    ...
    !!-----------------------------------------------------------------
    SUBROUTINE sOUT_OPT_VAR_Extract(nVAR,nHour,iHour,dSIM,sPAR,sOUT,VARopt_int)          
        !!EXTRACT hourly output for opt RESPONSE VARIABLES
        !!ARGUMENTS:
        INTEGER, intent(in)     :: nVAR
        INTEGER, intent(in)     :: iHour,nHour
        INTEGER, intent(in)     :: VARopt_int(nVAR,3)
        REAL(KIND=8), intent(out)    :: dSIM(nHour,nVAR)
        !    REAL(KIND=8) CO2_ISO_inp  !!cumulative CO2 isotope (e.g., C14 or C13) at the beginning of the time-step
        !    REAL(KIND=8), intent(in)     :: vENZ(2)      !!specific ENZ activity, ligninase & cellulase
        TYPE(sMEND_PAR),intent(in):: sPAR       
        TYPE(sMEND_OUT),intent(in):: sOUT
        ...
        do j = 1,nVAR                                 
            ...
            select case (k)  !!see "MEND.ini"
                ...
                case (13)    !! percent of MOC = MOC/SOC  !!7/26/2020
                    dSIM(iHour,j) = sOUT%CPOOL%MOMT/(sOUT%CPOOL%POMT + sOUT%CPOOL%MOMT)
                case (14)    !!NH4
                ...
                case (26)   !!Denitrification rate
                    dSIM(iHour,j) = sOUT%MNFLUX%Nitrifnet
                case (27)   !!N2O efflux
                ...
                case (35)   !!ENZ activity for PTASE
                    dSIM(iHour,j) = sOUT%CPOOL%PTASE*sPAR%VdP(1)
				!	write(*,*)sOUT%CPOOL%PTASE,sPAR%VdP(1)
                ...
                case (41)   !!Active fraction of microbes 
                    dSIM(iHour,j) = sOUT%MNFLUX%CO2_ovflow_MBD_N
                case (42)   
                    dSIM(iHour,j) = sOUT%CP%MB
                case (43)   
                    dSIM(iHour,j) = sOUT%MNPOOL%DIP+sOUT%MNPOOL%LIP
                case (44)   
                    dSIM(iHour,j) = sOUT%PPOOL%TM
                case default
                    dSIM(iHour,j) = sOUT%MNFLUX%CO2_gmo
            end select
        end do
    
    END !!subroutine sOUT_OPT_VAR_Extract
    !!-----------------------------------------------------------------
    SUBROUTINE sOUT_OPT(nh,dSIM_h,nt,dSIM_t,sDate_beg,sDate_end,tstep,flag_avg)
        ...
        REAL(KIND=8) dSIM_h(nh)
        REAL(KIND=8) dSIM_t(nt,2)  !!mean & sd
    ...
    SUBROUTINE sOUT_Day2Mon(nday,dSIM_d,nt,dSIM_t,sDate_beg,sDate_end,tstep,flag_avg)
        ...
        INTEGER iyr0, imo0, ida0, iyr, imo
        INTEGER nda_in_ym
        INTEGER j,nda,nmo, ibeg, iend,nbe
    ...
    !!-----------------------------------------------------------------
    SUBROUTINE sCOFI_PAR(nPAR, PAR_names, sFile_in,sFile_out, nSA)   
        INTEGER,        intent(in)  :: nPAR
        CHARACTER(*),   intent(in)  :: PAR_names(nPAR)
        CHARACTER(*),   intent(in)  :: sFile_in, sFile_out
        INTEGER,        intent(out) :: nSA


        INTEGER iFpar0, iFpar, eof, id

        REAL(KIND=8) fObj0,fObj_cr(2)  !!upper and lower bound for COFI
        REAL(KIND=8) xx(nPAR)
        CHARACTER(len=10)sRead1
        CHARACTER(len=200)format100, format101
        CHARACTER(len=1000)sRead

        write(format101,*)"(a10,",nPar,"(a15))"
        write(format100,*)"(f10.4,",nPAR,"(f15.8))"
        iFpar0 = 301                                    
        iFpar = 302                                     
        open(unit = iFpar0, file = trim(sFile_in) , status = 'old')
        open(unit = iFpar, file = trim(sFile_out) , status = 'unknown')
                                                        
        read(iFpar0, *)sRead,fObj_cr(1:2)  !!Critical Objective Function Value
        write(iFpar,'(A20,2f10.4)')sRead,fObj_cr(1:2)
        write(*,'(A20,2f10.4)')sRead,fObj_cr(1:2)
        read(iFpar0, '(2A10,A)')sRead1,sRead1,sRead      !!field names
        write(iFpar,format101)sRead1,PAR_names(1:nPAR)
        !! fOBJ            LF0             r0           fINP             VP
        nSA = 0  !!number of lines of PAR
        DO
            read(iFpar0, *, iostat = eof) id, fObj0,xx(1:nPAR)
            ...
            ELSE
                IF(fObj0.le.fObj_cr(1).and.fObj0.gt.fObj_cr(2)) then  !!fObj = rRead(1)
                    nSA = nSA+1
                    write(iFpar, format100)fObj0,xx
                !                ELSE
                !                    print*,">>>Skip this Line!"
                END IF
            END IF
    ...
    !!-----------------------------------------------------------------
    SUBROUTINE sSOBOL_VAR_OBJ0(sFile_t0,sFile_t,nRow_skip,nVAR,sDate_beg,sDate_end,tstep,var_names,fobj)
        !!Calculate objective function values for Sobol Sensitivity analysis
        !!sFile_t0: file name for data from parameter median/mean
        !!sFile_t: file name for data from a parameter sample
        ...
        CHARACTER(len=*)sFile_t0,sFile_t
        CHARACTER(len=8) sDate_beg,sDate_end
        CHARACTER(len=3000)var_names
        REAL(KIND=8) fobj(nVAR)

        !!LOCAL VARIABLES:
        !    INTEGER iyr0, imo0, ida0, iyr, imo, iyear,imonth,iday
        !    INTEGER nda_in_ym
        INTEGER i,j
        INTEGER nda,nmo,nyr,nbe
        INTEGER ip1,ip2
        !    CHARACTER(LEN=10) sDateHr
        CHARACTER(len=8) sDate
        !    CHARACTER(len=6) sYM
        ...
        REAL(KIND=8), ALLOCATABLE:: dSIM_t(:,:) !!days in 1 month, at most = 31, hours in 1 month, at most =24*31
        !    REAL(KIND=8), DIMENSION(366,nVAR)::dSIM_h3 !!days in 1 year, at most =366, hours in 1 year, at most =366*24
        !    REAL(KIND=8), DIMENSION(nVAR)   ::dSIM_t
        !    dSIM_t0 = const_FillValue
        ...
        !    end if
        !
        !    open(ip2,file=sFile_t,status='unknown')
        ...
        !
        !    CALL sDate2YMD(sDate_beg,iyr0,imo0,ida0)
        open(ip1,file=sFile_t0,status='old')
        open(ip2,file=sFile_t,status='old')
        read(ip1,'(a)')sRead1      !1st line: time period
        read(ip2,'(a)')sRead1      !1st line: time period
        read(ip1,'(a10,a)')sRead1,var_names  !2nd line: head
        read(ip2,'(a)')sRead1  !2nd line: head
        !    if(tstep.eq.1) then
        !        write(ip2,'(a10,a)')"Day",sRead2
        !    elseif(tstep.eq.2) then
        !        write(ip2,'(a10,a)')"Mon",sRead2
        !    elseif(tstep.eq.4) then
        !        write(ip2,'(a10,a)')"Year",sRead2
        !    end if
        ...
        if (nbe >0) then
            ALLOCATE(dSIM_t0(nbe,nVAR))
            ALLOCATE(dSIM_t(nbe,nVAR))
            do i = 1,nbe
                read(ip1,*)sDate,dSIM_t0(i,1:nVAR)
                read(ip2,*)sDate,dSIM_t(i,1:nVAR)
            end do

            do j = 1,nVAR
                fobj(j) = fMSE(nbe, dSIM_t0(:,j), dSIM_t(:,j), 0, const_FillValue)  !!fMSE_Normalize()
            end do
        else
            fobj(1:nVAR) = const_FillValue
        end if
        close(ip1)
        close(ip2)
    ...
    END !!subroutine sSOBOL_VAR_OBJ0


    !!-----------------------------------------------------------------
    SUBROUTINE sSOBOL_VAR_OBJ(itype,xx, sPAR, sINI, sOUT, sFile_in,sFile_out, &
        nRow_skip, tstep,flag_avg, &
        nPAR, nVAR, nt,dSIM_t0,Sobol_obj)
        !!Calculate objective function values for Sobol Sensitivity analysis
        !!itype = 0: calculate and return daily VARiables (dSIM_t0)
        !!itype = 1: calculate and return daily VARiables, then calculate Sobol Objective function values (Sobol_obj)
        !!sFile_in: hourly variables data
        !!sFile_out: daily variables data
        !!nRow_skip: >=2, first nRow to be skipped: 1st-row: time period; 2nd-row: head; >=3rd-row: others, e.g., values at t=0
        !!nday: # of days
        !!nVAR: # of variables (columns) in datafile
        !!nh: # of hours
        ...
        REAL(KIND=8), intent(in) :: xx(nPAR)
        REAL(KIND=8), intent(out):: Sobol_obj(nVAR)

        INTEGER itype, nPAR, nVAR, nt
        INTEGER nRow_skip,tstep, flag_avg !!nh, nt,
        CHARACTER(len=*)sFile_in,sFile_out
        !    CHARACTER(len=8) sDate_beg,sDate_end

        INTEGER j
        !    CHARACTER(len=3000)var_names
        REAL(KIND=8) fobj
        REAL(KIND=8) dSIM_t0(nt,nVAR)
        REAL(KIND=8) dSIM_t(nt,nVAR)

        fObj = fMEND_OBJ(xx, sPAR, sINI, sOUT)
        !                write(*,*)"pid=",pid,fObj,sINI%iError
        CALL subMEND_Files_Close(sINI)

        if (sINI%iError < 0) then
            if(itype.eq.0) then
                dSIM_t0 = dabs(const_FillValue)
            else
                Sobol_obj = dabs(const_FillValue)
            end if
        else
            CALL sOUT_ALL_tscale(sFile_in,sFile_out,nRow_skip,nVAR,&
                sINI%sDate_beg_sim,sINI%sDate_end_sim,tstep,flag_avg)
            CALL system('gzip -f '//sFile_in)

            CALL sSOBOL_VAR_Read(dSIM_t,sFile_out,nRow_skip,nt,nVAR)
            if(itype.eq.0) then
                dSIM_t0 = dSIM_t
            else
                do j = 1,nVAR
                    Sobol_obj(j) = fMSE(nt, dSIM_t0(:,j), dSIM_t(:,j), 0, const_FillValue)  !!fMSE_Normalize()
                end do
            end if
        end if !!(sINI%iError < 0)
    END !!SUBROUTINE sSOBOL_VAR_OBJ
    !!-----------------------------------------------------------------
    SUBROUTINE sSOBOL_VAR_Read(dSIM_t,sFile_t,nRow_skip,nRow,nVAR)
        !!Read Output VARiables for Sobol Sensitivity analysis
        !!dSIM_t(nRow,nVAR): data read from sFilt_t
        !!sFile_t: file name for data from a parameter sample
        !!nRow_skip: >=2, first nRow to be skipped: 1st-row: time period; 2nd-row: head; >=3rd-row: others, e.g., values at t=0
        !!nday: # of days
        !!nVAR: # of variables (columns) in datafile
        ...
        !!sDate_beg: YYYYMMDD, beginning date
        !!tstep = 1(daily),2(monthly),3(seasonal),4(yearly)
        !!var_names: variable names in the 1st line
        !!ARGUMENTS:
        REAL(KIND=8), intent(out)    :: dSIM_t(nRow,nVAR)
        INTEGER, intent(in)     :: nRow_skip,nRow,nVAR
        CHARACTER(len=*), intent(in)     :: sFile_t
        !    CHARACTER(len=3000), intent(out)     :: var_names

        !!LOCAL VARIABLES:
        INTEGER i
        INTEGER ip2
        CHARACTER(len=8) sDate
        CHARACTER(len=3000)sRead1
    ...
    END !!subroutine sSOBOL_VAR_Read
    ...
    SUBROUTINE sOUT_ALL_tscale(sFile_hour,sFile_t,nRow_skip,nVAR,sDate_beg,sDate_end,tstep,flag_avg)
        ...
        INTEGER nda_in_ym
        INTEGER i,j
        INTEGER nda,nmo,nyr, ibeg, iend,nbe !!nhr
        INTEGER ip1,ip2
        CHARACTER(LEN=10) sDateHr
        CHARACTER(len=8) sDate
        CHARACTER(len=6) sYM
        CHARACTER(len=6000)sRead1,sRead2      
        CHARACTER(len=200)format1
        REAL(KIND=8), DIMENSION(24,nVAR)::dSIM_h1
        REAL(KIND=8), DIMENSION(31,nVAR)::dSIM_h2 !!days in 1 month, at most = 31, hours in 1 month, at most =24*31
        REAL(KIND=8), DIMENSION(366,nVAR)::dSIM_h3 !!days in 1 year, at most =366, hours in 1 year, at most =366*24
        REAL(KIND=8), DIMENSION(nVAR)   ::dSIM_t
        ...
        !
        CALL sDate2YMD(sDate_beg,iyr0,imo0,ida0)
        open(ip1,file=sFile_hour,status='old')        
        read(ip1,'(a)')sRead1      !1st line: time period
        write(ip2,'(a)')sRead1
        read(ip1,'(a10,a)')sRead1,sRead2  !2nd line: head     
        if(tstep.eq.1) then
            write(ip2,'(a10,a)')"Day",sRead2
        elseif(tstep.eq.2) then
            write(ip2,'(a10,a)')"Mon",sRead2
                ...
                if(nda > 0) then
                    do i = 1,nda
                        ...
                        do j=1,24 !!24 hours in 1 day
                            read(ip1,*)sDateHr,dSIM_h1(j,1:nVAR)
                        end do
                        do j=1,nVAR
                            if(flag_avg.eq.1) then
                        ...
                        write(ip2,format1)sDate,dSIM_t
                    end do
                end if !! nda > 0
            case (2) !!# convert daily to monthly
                if(nmo > 0) then
                    ...
                    do i=1,nmo
                        ...
                        iend = nda_in_ym
                        do j = ibeg,iend
                            read(ip1,*)sDate,dSIM_h2(j,1:nVAR)
                        end do
                        do j=1,nVAR
                            if(flag_avg.eq.1) then
            ...
            case(4) !!# convert daily to yearly
                if(nyr>0) then
                    do i=1,nyr
                        ...
                        do j = ibeg,iend
                            read(ip1,*)sDate,dSIM_h3(j,1:nVAR)
                        end do
        ...
        close(ip1)
        close(ip2)
    END !!subroutine sOUT_ALL_tscale
    !!-----------------------------------------------------------------
    SUBROUTINE sOUT_tscale(sINI,sDate_beg,sDate_end)
        !!# Convert OUTPUTS from HOURLY to DAILY & MONTHLY for all state variables & fluxes
        !!ARGUMENTS:
        !    CHARACTER(LEN = *)dirout
        TYPE(sMEND_INI), intent(in) :: sINI
        CHARACTER(LEN = 8), intent(in) :: sDate_beg,sDate_end
    
        !!LOCAL VARIABLES:
        CHARACTER(LEN = 200) sFile_inp, sFile_out_day, sFile_out_mon, sFile_out_year
        INTEGER nRow_skip, nVAR, tstep,flag_avg, nstr
        CHARACTER(len=8) file_day, file_mon
        CHARACTER(len=9) file_year
        ...
        sFile_inp = sINI%sFile_VAR_hour
        nstr = len(trim(sFile_inp)) - 8
        sFile_out_day = sFile_inp(1:nstr)//file_day
        sFile_out_mon = sFile_inp(1:nstr)//file_mon
        sFile_out_year = sFile_inp(1:nstr)//file_year
        ...
        CALL sOUT_ALL_tscale(sFile_inp,sFile_out_day,nRow_skip,nVAR, sDate_beg, sDate_end,tstep,flag_avg) 
        CALL system('gzip -f '//sFile_inp)
    
        nRow_skip=3; nVAR=const_nPOOL*5 + const_nPOOL_MN; tstep=2; flag_avg=1
        CALL sOUT_ALL_tscale(sFile_out_day,sFile_out_mon,nRow_skip,nVAR, sDate_beg, sDate_end,tstep,flag_avg) 
    
        nRow_skip=3; nVAR=const_nPOOL*5 + const_nPOOL_MN; tstep=4; flag_avg=1
        CALL sOUT_ALL_tscale(sFile_out_day,sFile_out_year,nRow_skip,nVAR, sDate_beg, sDate_end,tstep,flag_avg) 
    
        print*,">>>[2] FLUXes:"                 
        sFile_inp = sINI%sFile_FLX_hour
        nstr = len(trim(sFile_inp)) - 8
        sFile_out_day = sFile_inp(1:nstr)//file_day
        sFile_out_mon = sFile_inp(1:nstr)//file_mon
        sFile_out_year = sFile_inp(1:nstr)//file_year

        nRow_skip=2; nVAR=const_nFLUX*3 + const_nFLUX_MN; tstep=1; flag_avg=1                              
        CALL sOUT_ALL_tscale(sFile_inp,sFile_out_day,nRow_skip,nVAR, sDate_beg, sDate_end,tstep,flag_avg)  
        CALL system('gzip -f '//sFile_inp)
    
        nRow_skip=2; nVAR=const_nFLUX*3 + const_nFLUX_MN; tstep=2; flag_avg=1
        CALL sOUT_ALL_tscale(sFile_out_day,sFile_out_mon,nRow_skip,nVAR, sDate_beg, sDate_end,tstep,flag_avg) 
    
        nRow_skip=2; nVAR=const_nFLUX*3 + const_nFLUX_MN; tstep=4; flag_avg=1
        CALL sOUT_ALL_tscale(sFile_out_day,sFile_out_year,nRow_skip,nVAR, sDate_beg, sDate_end,tstep,flag_avg) 
    
        print*,">>>[3] PARAMETERs:"      
        sFile_inp = sINI%sFile_PAR_hour
        nstr = len(trim(sFile_inp)) - 8
        sFile_out_day = sFile_inp(1:nstr)//file_day
        sFile_out_mon = sFile_inp(1:nstr)//file_mon
        sFile_out_year = sFile_inp(1:nstr)//file_year
        ...
        sFile_inp = sINI%sFile_RATE_hour
        nstr = len(trim(sFile_inp)) - 8
        sFile_out_day = sFile_inp(1:nstr)//file_day
        sFile_out_mon = sFile_inp(1:nstr)//file_mon
        sFile_out_year = sFile_inp(1:nstr)//file_year
    ...
    !!-----------------------------------------------------------------
    SUBROUTINE subMEND_Files_Open(sINI)                 
        TYPE(sMEND_INI), intent(inout) :: sINI
        ...
        open(unit= sINI%iFout_SIM_day, file = sfilename_full , status = 'unknown')     
        ...
        open(unit= sINI%iFout_SIM_mon, file = sfilename_full , status = 'unknown') 
        ...
        open(unit= sINI%iFout_VAR_hour, file = trim(sINI%sFile_VAR_hour) , status = 'unknown')  
        write(sINI%iFout_VAR_hour,*)"Simulation_Period = ",sINI%sDate_beg_sim, " -- ",sINI%sDate_end_sim
        write(sINI%iFout_VAR_hour,'(a10,200a20)')"Hour",(trim(sINI%Name_POOL(i))//"_C",i=1,const_nPOOL), &
            (trim(sINI%Name_MNPOOL(i)),i=1,const_nPOOL_MN), &
            (trim(sINI%Name_POOL(i))//"_N",i=1,const_nPOOL),(trim(sINI%Name_POOL(i))//"_P",i=1,const_nPOOL), &
            (trim(sINI%Name_POOL(i))//"_CN",i=1,const_nPOOL),(trim(sINI%Name_POOL(i))//"_CP",i=1,const_nPOOL)

        !      sINI%sFile_FLX_hour = trim(sINI%dirout)//"FLX_hour.out"         
        open(unit = sINI%iFout_FLX_hour, file = trim(sINI%sFile_FLX_hour) , status = 'unknown')   
        write(sINI%iFout_FLX_hour,*)"Simulation_Period = ",sINI%sDate_beg_sim, " -- ",sINI%sDate_end_sim
        write(sINI%iFout_FLX_hour,'(a10,200a20)')"Hour",(trim(sINI%Name_FLUX(i))//"_C",i=1,const_nFLUX), &
            (trim(sINI%Name_MNFLUX(i)),i=1,const_nFLUX_MN), &
            (trim(sINI%Name_FLUX(i))//"_N",i=1,const_nFLUX),(trim(sINI%Name_FLUX(i))//"_P",i=1,const_nFLUX) 

        !      sINI%sFile_RATE_hour = trim(sINI%dirout)//"RATE_hour.out"      
        open(unit = sINI%iFout_rate_hour, file = trim(sINI%sFile_RATE_hour) , status = 'unknown')  
        write(sINI%iFout_RATE_hour,*)"Simulation_Period = ",sINI%sDate_beg_sim, " -- ",sINI%sDate_end_sim
        write(sINI%iFout_RATE_hour,'(a10,100a20)')"Hour",(trim(sINI%Name_RATE(i)),i=1,const_nRATE)

        !      sINI%sFile_PAR_hour = trim(sINI%dirout)//"PAR_hour.out"                 
        open(unit = sINI%iFout_par_hour, file = trim(sINI%sFile_PAR_hour) , status = 'unknown')
        write(sINI%iFout_PAR_hour,*)"Simulation_Period = ",sINI%sDate_beg_sim, " -- ",sINI%sDate_end_sim
        write(sINI%iFout_PAR_hour,'(a10,100a20)')"Hour",(trim(sINI%Name_PAR(i)),i=1,const_nPAR)

    END !!subMEND_Files_Open
                                                                                                                    
    !!-----------------------------------------------------------------
    SUBROUTINE subMEND_Files_Close(sINI)
        TYPE(sMEND_INI), intent(in) :: sINI
    ...
    SUBROUTINE sINP_Read(nfile,sfilename,dirinp,ststep,is_total,nMon,nHour,rINP)
        ...
        REAL(KIND=8),         intent(inout)  :: rINP(nHour)
        ...
        elseif(trim(ststep).eq."monthly") then !!only 1 file is allowed
            ...
            do j=1,nmons
                ...
                do lp = 1,ndays*24
                    k = k+1
                    if(is_total.eq.1) then
                        rINP(k) = max(0d0,rRead)/DBLE(ndays*24)
                    else
                        rINP(k) = max(0d0,rRead)
                    end if
                end do
...
!!-----------------------------------------------------------------
    ...
    SUBROUTINE TEST()
        ...
        !    REAL(KIND=8), dimension(8) :: v = (/ 2,4,4,4,5,5,7,9 /)
        !    REAL(KIND=8) sd
        !    sd = fSTDDEV(6,v(2:7),const_FillValue)
        !    print *, "std dev = ", sd

        !    REAL(KIND=8) fSWC2SWP
        INTEGER i,j, ni, nj !!iRead(10)
        REAL(KIND=8) rRead(10)
        CHARACTER(len=20) sRead(10),units
        character(len=200):: sFile_inp,sFile_out
        !    REAL(KIND=8) fSWPsat1,fSWP_dec1
        REAL(KIND=8) SWCres,SWCsat,alpha,rn
        REAL(KIND=8) SWC,SWP
        ...
        do i=1,ni
            ...
            do j=1,nj
                SWC = rRead(j)
                SWP = fSWC2SWP(SWC,SWCres,SWCsat,alpha,rn)
                rRead(nj+j) = SWP
            end do
...
!REAL(KIND=8) function fMEND_OBJ0(xx, sPAR, sINI, sOUT)
!    USE STRUCT_MEND
!    TYPE(sMEND_PAR), intent(inout) :: sPAR
!    TYPE(sMEND_INI), intent(inOut) :: sINI
!    TYPE(sMEND_OUT) sOUT
!    TYPE(sMEND_INP) sINP
!    REAL(KIND=8) sum1, fNSE !function
!    REAL(KIND=8) xx(sPAR % nPar)
!    INTEGER nObj
!
